---
kind: method
statement: "Pugh controlled convergence with reversed sensitivity: datum = strongest viable rival; iterate with the winner as new datum; then hunt the world where the winner loses."
---
...
adds them to the set and scores them, and the ideation the matrix provokes is
reported as a large part of its value rather than a side effect.

OURS RUNS IT LATER AND NARROWER, ON PURPOSE. Hybridising before the choice
produces a new candidate that owes a full composition and a full scoring.
Grafting after the choice is a small yes or no per axis against a fixed target.

THE PRICE OF THAT CHOICE IS NAMED RATHER THAN HIDDEN. Grafting late can only
strengthen the shape that won. It cannot find a shape nobody drew, which is
exactly what hybridising early is for.

FOUND MISSING 2026-08-19, when the owner asked where the graft lived and every
candidate place turned out to only record, only score, or only hunt for where
the winner loses.
