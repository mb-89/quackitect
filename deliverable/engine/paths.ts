...
const UPSTREAM_FILE = ["deliverable", "vendor", "upstream", "upstream.json"] as const;
...
function containedIn(root: string, p: string, source: string): string {
  ...
  throw new Rejection({
    clause: CLAUSES.PATH_ESCAPE,
    expected: "a path inside the project root (root-relative, e.g. spec/x.md)",
    got: p,
    remedy: {
      tool: "se_file_list",
      args: { dir: "." },
 ...
 *  no tree owns it. */
export const METHOD_FILES = ["AGENTS.md", "CLAUDE.md", ".github/instructions/protocol.instructions.md"] as const;
...
export function recordOwnerOf(rel: string): { container: "iterations" | "expeditions"; id: string } | undefined {
  const m = /^spec\/(iterations|expeditions)\/([^/]+)\//.exec(slashed(rel));
