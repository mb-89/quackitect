...
function seededStepIds(recordRoot: string, only: string | undefined): Set<string> | undefined {
  const dir = join(recordRoot, "spec", "iterations");
  try {
...
function recordDirFor(recordRoot: string, record: string | undefined): string | undefined {
  let names: string[];
  try {
    names = readdirSync(join(recordRoot, "spec", "iterations"));
  } catch {
    return undefined;
  }
...
function unclaimedEngineFiles(recordRoot: string, claimed: Set<string>): string[] {
  const base = join(recordRoot, "deliverable", "engine");
  const out: string[] = [];
  const walk = (d: string, rel: string): void => {
    ...
    for (const e of entries) {
      ...
      const full = `deliverable/engine/${r}`;
...
function refProblems(name: string, meta: TemplateMeta, args: FieldArgs, content: string, corpus?: TraceNode[], root?: string): string[] {
  ...
  // the wrong type.
  //
  // A DECLARED-COLUMN TABLE IS THE SAME CASE, and it is a second way in rather
  // than the same one. `columns` names frontmatter KEYS, so a row is one node
  // followed by that node's field values — and a value like an element id is
  // not this field's artifact. Either tell is enough.
  const nodeTable = meta.editor === "node-table";
  const valueCells = nodeTable || args.writes !== "" || args.columns.length > 0;
