...
// (deliverable/machines/main.canvas). start and end are MECHANICAL
...
export function mainMachinePath(root: string): string {
  return join(root, "deliverable", "machines", "main.canvas");
}
...
export class Session {
  ...
  requestReload(): Record<string, unknown> {
    ...
    if (r.status !== 0) {
      throw new Rejection({
        ...
        remedy: {
          tool: "se_file_read",
          args: { path: "deliverable/engine/tools.ts" },
  ...
  private loadFormTemplate(name: string): FormTemplate {
    ...
    if (!existsSync(tplAbs)) {
      throw new Rejection({
        ...
        got: "not found",
        remedy: { tool: "se_file_glob", args: { glob: "deliverable/machines/forms/*" }, note: "the templates that exist" },
  ...
  private formHome(name: string): {
    ...
    const recRel = ["spec", "expeditions", this.bound.id];
    return {
      template,
