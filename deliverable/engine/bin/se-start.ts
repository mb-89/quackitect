...
export function recordOnTrunk(root: string, iteration: string): string {
  const rel = `spec/iterations/${iteration}/record.md`;
...
function fetchRefs(iteration: string): void {
  ...
  if (recordOnTrunk(ROOT, iteration) === "") {
    die(
      "fetch",
      `refs are up to date and this repository has no spec/iterations/${iteration}/record.md — a record is a folder on trunk, so that path IS the iteration`,
    );
  }
...
function launch(iteration: string, agent: string, pid: number): void {
  const cage = join(ROOT, "deliverable", "cage", "claude-settings.json");
  ...
  const card = join(ROOT, "guidance", "method", "cloud-runner.md");
