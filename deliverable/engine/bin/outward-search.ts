...
/** WHETHER A SOURCE IS ONE OF OURS.
 *
 *  This used to be a `project/` prefix test, back when every root-relative
 *  path opened with the opened folder's name. The levels collapsed, the prefix
 *  went, and the test went quiet — so the shape it stood for is written out.
 *
 *  A URL is never ours, and it is checked first so a query string full of
 *  slashes cannot be read as a path. */
function citesUs(source: string): boolean {
  if (/^[a-z][a-z0-9+.-]*:\/\//i.test(source)) return false;
  const p = source.split("\\").join("/");
  if (/^(product)\//.test(p) || p.startsWith(".se/")) return true;
  if (/^(deliverable|guidance|spec|scratchpad)\//.test(p)) return true;
  // A folder nobody listed is still ours if the file is really there.
  return existsSync(join(root, p));
}

const root = argValue("--root") ?? process.cwd();
const optionDir = join(root, "spec", "trace", "option");
