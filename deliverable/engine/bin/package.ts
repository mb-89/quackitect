...
const version = String(JSON.parse(readFileSync(join(root, "deliverable", "package.json"), "utf8")).version);
...
mkdirSync(join(stage, "spec"), { recursive: true });

// The README inside is the ENTRY document, not this repo's own.
const abbr = brand.abbr ?? brand.name.slice(0, 2).toUpperCase();
const readme = readFileSync(join(root, "deliverable", "brand", "README.entry.md"), "utf8")
