...
export function suiteFiles(root: string): string[] {
  const dir = join(root, "deliverable", "tests");
  try {
    return readdirSync(dir)
      .filter((f) => f.endsWith(".test.ts"))
      .map((f) => `deliverable/tests/${f}`)
      .sort();
  } catch (e) {
...
export function mapChangedToTests(root: string, changed: string[]): { mapped: string[]; unmapped: string[] } {
  ...
  for (const rel of changed) {
    ...
    const m = p.match(/^deliverable\/engine\/([\w-]+)\.ts$/);
    const candidate = m !== null ? `deliverable/tests/${m[1]}.test.ts` : undefined;
...
function isDocument(rel: string): boolean {
  ...
  // The leading segment counts — see the note in guard.ts on isCorpusNode.
  return /(^|\/)(spec|machines|guidance)\//.test(p);
}
