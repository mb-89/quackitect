...
// Evidence lives in frontmatter. see dsp-method-compilation.md#a-drawn-state-speaks-the-matrixs-language
...
function refuseBadRow(row: RigorMatrixRow): void {
  ...
  // see dsp-method-compilation.md#when-a-state-may-declare-no-evidence
  if (
...
export function matrixDir(root: string): string {
  return join(root, "deliverable", "machines", "rigor_matrix");
}
...
export function assertFloor(matrix: RigorMatrix, column: ChangeColumn): void {
  ...
  throw new Rejection({
    ...
    remedy: {
      tool: "se_file_glob",
      args: { glob: `deliverable/machines/rigor_matrix/rows/*${struck[0].name}.md` },
