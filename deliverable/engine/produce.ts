...
const PROMPT_LAYER_IN_GITHUB = join(".github", "instructions", "protocol.instructions.md");
 ...
 *  travelled. The levels have since collapsed, so there is a single
 *  `.claude`: it is committed, it is the only thing a fresh clone reads at
 *  session start, and it is what fires the arrival. It travels.
 *
...
export function travels(root: string, src: string): boolean {
  ...
  // THE RECORDS STAY HOME. `spec` is this tree's own expeditions, its
  // iterations AND its trace, and every part of it describes work the receiver
  // never did.
  if (parts[0] === "spec") return false;
  return true;
}

const BRAND = join(...BRAND_PARTS);
const README_TEMPLATE = join("deliverable", "brand", "README.entry.md");
