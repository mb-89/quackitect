...
function lexString(src: string, i: number): { tok: Tok; next: number } {
  ...
  if (j >= src.length) {
    throw new Rejection({
      ...
      got: src,
      remedy: { tool: "se_file_read", args: { path: "spec/bases-syntax.md" }, note: "strings take single or double quotes" },
      source: SRC,
    });
  }
...
function lex(src: string): Tok[] {
  ...
  while (i < src.length) {
    ...
    throw new Rejection({
      ...
      remedy: { tool: "se_file_read", args: { path: "spec/bases-syntax.md" }, note: "section 7 lists every operator" },
      source: SRC,
    });
  }
  out.push({ k: "end" });
...
class Parser {
      ...
      if (this.eat(".")) {
        ...
        if (t.k !== "id") {
          throw new Rejection({
            ...
            remedy: { tool: "se_file_read", args: { path: "spec/bases-syntax.md" }, note: "section 4 covers property references" },
            source: SRC,
          });
        }
        this.pos++;
    ...
    for (;;) {
      ...
      if (key.k !== "str" && key.k !== "id") {
        throw new Rejection({
          ...
          got: JSON.stringify(this.src),
          remedy: { tool: "se_file_read", args: { path: "spec/bases-syntax.md" }, note: "section 8 covers objects" },
          source: SRC,
        });
      }
      this.pos++;
  ...
  private primary(): Node {
    ...
    throw new Rejection({
      ...
      remedy: { tool: "se_file_read", args: { path: "spec/bases-syntax.md" }, note: "section 8 lists every type literal" },
      source: SRC,
    });
  }
}
