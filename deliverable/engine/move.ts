...
function refPairs(fromRel: string, toRel: string): RefPair[] {
  ...
  // THE VAULT FORM AND THE ROOT-RELATIVE FORM ARE ONE STRING since the folder
  // levels collapsed, so there is no second text form to repair — the pair
  // above already covers it. The WIKI form is still its own spelling.
  if (fromRel.endsWith(".md") && toRel.endsWith(".md")) {
    pairs.push({ old: `[[${fromRel.slice(0, -3)}`, new: `[[${toRel.slice(0, -3)}`, prose_only: true });
  }
