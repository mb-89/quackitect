...
function loadWatchConfig(root: string): VaultWatchConfig {
  try {
    const raw = JSON.parse(readFileSync(join(root, ".se", "quack-watch.json"), "utf8")) as Partial<VaultWatchConfig>;
    return {
...
function vaultPath(vault: Vault, path: string): string | undefined {
  // THE VAULT ROOT IS THE REPOSITORY ROOT since the folder levels collapsed,
  // so a root-relative path IS the vault-relative one. What decides membership
  // is the watch config, which it always was; the prefix test only ever stood
  // in for the folder boundary.
