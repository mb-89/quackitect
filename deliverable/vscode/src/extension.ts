...
// Card numbers are muscle memory (deliverable/views/cards.md), so a slot is reserved
...
// THE OPENED FOLDER IS THE ROOT. It used to be a folder one level down, and
// that level was collapsed away: the repository root now holds the work.
//
// THE PARENT IS STILL TRIED, so a person who opens a folder one level down
// still lands on the right root. A root is recognised by the engine it
// carries, never by its name.
function projectRoot() {
  const isRoot = (p) => existsSync(path.join(p, "deliverable", "engine", "bin", "se-mcp.ts"));
...
function placeVoiceProjections(root, opened) {
  ...
  const GENERATED = "description: Generated from guidance/voice.md. Edit that file, never this one.";
  try {
    const voice = readFileSync(path.join(root, "guidance", "voice.md"), "utf8");
...
function startServer(root, runner) {
  const entry = path.join(root, "deliverable", "engine", "bin", "se-mcp.ts");
...
// ── THE CARDS ────────────────────────────────────────────────────────────
// Read from the engine, never listed here: deliverable/views/cards.md is the truth,
...
async function showScaleHelp(which, level) {
  ...
  const rows = list
    .map((l) => {
      ...
      // THE ABBREVIATION ONLY. The scale is categorical, and the number
      // behind it is machinery a reader cannot act on (owner ruling
      // 2026-08-18).
...
function agentLaunch(root) {
  const cageDir = path.join(root, "deliverable", "cage");
...
async function openCard(n) {
  ...
  if (!shown(card)) {
    void vscode.window.showInformationMessage(`$PRODUCT$: deliverable/views/cards.md declares no card ${n}.`);
    return;
  }
...
function showAttach() {
  const root = projectRoot();
  if (root === null) return;
  const doc = vscode.Uri.file(path.join(root, "deliverable", "vscode", "ATTACH.md"));
