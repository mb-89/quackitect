...
import { itPinRel, itSeed, itSeededRel, pinIteration } from "../engine/iterations.ts";
import { generateIterations, generateSeeded, pinnedCanvas } from "../engine/iterations-draw.ts";
...
test("any state's form is fetchable by its machine — the walk elsewhere", () => {
  ...
  const inst = join(it.path, "spec", "iterations", it.id, "evidence", "onboard-retro.md");
...
test("the bless pins the machine and it grows in place — no wrapper, fills carried", async () => {
  ...
  const retroForm = readFileSync(join(root, "spec", "iterations", id, "evidence", "onboard-retro.md"), "utf8");
