...
import { fileRead } from "../engine/files.ts";
...
test("a truncating shape refuses before the spawn, and names the verb that was wanted", async () => {
  ...
  const cases: [string, string][] = [
    ["node --test deliverable/tests/pull.test.ts | Select-String -Pattern fail", "se_test"],
...
test("the classifier names the lane tool for each harvested shell shape", () => {
  ...
  assert.equal(classifyCommand("Get-Content engine/toll.ts")?.tool, "se_file_read");
...
function productRoot(): string {
  ...
  for (const m of ["pull", "files", "search"]) {
    writeFileSync(join(root, "deliverable", "engine", `${m}.ts`), `// ${m}\n`);
    writeFileSync(join(root, "deliverable", "tests", `${m}.test.ts`), `// ${m} test\n`);
  }
  const g = (...a: string[]): void => {
...
test("a change that maps to a test file is answered by that file, and the decision says so", () => {
  ...
  writeFileSync(join(root, "deliverable", "engine", "pull.ts"), "// changed\n");
  const d = decideScope(se, root, false);
  assert.equal(d.scope, "scoped");
  assert.deepEqual(d.files, ["deliverable/tests/pull.test.ts"], "the engine picked the file, not the caller");
...
test("piecemeal past the flip becomes the battery, and the reason says the count", () => {
  ...
  for (let i = 0; i < threshold; i++) {
    testRecord(se, root, true, `scope-${i}`, [`deliverable/tests/f${i}.test.ts`]);
  }
  writeFileSync(join(root, "deliverable", "engine", "pull.ts"), "// changed\n");
  const d = decideScope(se, root, false);
...
test("the change-to-test map is by name and refuses to guess", () => {
  const root = productRoot();
  const r = mapChangedToTests(root, ["deliverable/engine/pull.ts", "deliverable/tests/files.test.ts", anyGuidanceDoc()]);
  assert.deepEqual(r.mapped, ["deliverable/tests/files.test.ts", "deliverable/tests/pull.test.ts"]);
...
test("the full battery formats before preflight and stops on format failure", () => {
  ...
  const preflightAt = battery.indexOf('"deliverable/engine/bin/preflight.ts"');
