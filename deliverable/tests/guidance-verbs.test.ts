...
function registered(): Set<string> {
  const dir = join(ROOT, "deliverable/engine");
...
// deliverable/machines/stopat.md.
//
...
test("every file path the guidance names resolves from the project root", () => {
  ...
  for (const p of guidancePages()) {
    ...
    for (const m of text.matchAll(PATH_LIKE)) {
      ...
      const near = ["deliverable", "guidance", "deliverable/machines"].find((pre) => existsSync(join(ROOT, pre, ref)));
...
test("every refusal clause is documented, and every documented clause exists", () => {
  const engine = new Set([...readFileSync(join(ROOT, "deliverable/engine/errors.ts"), "utf8").matchAll(/"(SE-C-\d+)"/g)].map((m) => m[1]));
