...
const BRAND = join("deliverable", "brand", "brand.json");
const UPSTREAM = join("deliverable", "vendor", "upstream", "upstream.json");
...
describe("producing a vehicle", () => {
  ...
  test("it produces a complete tree under the new name, in a repository of its own", () => {
    ...
    assert.ok(existsSync(join(made.dest, "deliverable", "engine", "paths.ts")), "the engine travels");
    assert.ok(existsSync(join(made.dest, "guidance", "refusals.md")), "the guidance travels");
  ...
  test("it leaves the engine's own records at home and an empty place ready", () => {
    ...
    for (const generated of ["dist", join("scratchpad"), join(".obsidian"), join(".vscode")]) {
    ...
    assert.ok(
      existsSync(join(REPO, ".obsidian")),
      ".obsidian is tracked, so a vehicle not carrying it is a real exclusion rather than an absent source",
    );
  });

  test("the one file the arrival needs travels, and the generated one beside it does not", () => {
    const made = aVehicle();
    // THE .claude IS COMMITTED and is the only hook a fresh clone reads at
    ...
    //
    // IT USED TO BE TWO FOLDERS — a committed one at the repository root and a
    // generated one in the opened folder below it — and this test asked for the
    // first and against the second. The levels have since collapsed, so
    // there is one folder, it is the committed one, and it travels.
    assert.ok(existsSync(join(made.dest, ".claude", "settings.json")), "the cage that fires the arrival must travel");
    ...
    assert.ok(
      existsSync(join(made.dest, ".github", "instructions", "protocol.instructions.md")),
      "the Copilot host's prompt layer must travel, even though .github does not",
    );
    assert.ok(!existsSync(join(made.dest, ".github", "skills")), "and the rest of .github does not");
  });
  ...
  test("a failure past the copy leaves nothing behind", () => {
    ...
    const half = emptyDir();
    mkdirSync(join(half, "deliverable", "brand"), { recursive: true });
...
describe("a vehicle producing a project", () => {
  test("the project carries none of the method and one record naming its vehicle", () => {
    ...
    assert.ok(!existsSync(join(project, "deliverable", "engine")), "a driven tree carries no engine");
    assert.ok(!existsSync(join(project, "guidance")), "and none of the method");
  ...
  test("a driving tree that cannot say who it is refuses, rather than writing itself an identity", () => {
    ...
    mkdirSync(join(nameless, "deliverable", "brand"), { recursive: true });
...
describe("what a vehicle made its own", () => {
  ...
  test("it derives what the owner wrote, changed and removed", () => {
    ...
    writeFileSync(join(vehicle.dest, "guidance", "mine.md"), "# a card of my own\n", "utf8");
    ...
    assert.equal(how("guidance/mine.md"), "written");
