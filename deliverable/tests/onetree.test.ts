...
test("a bare pull at the container enters no iteration", async () => {
  ...
  for (const rec of itList(root)) {
    const abs = join(rec.path, `spec/iterations/${rec.id}/record.md`);
...
test("one open iteration is offered, never entered and never skipped past", async () => {
  ...
  const abs = join(root, `spec/iterations/${only}/record.md`);
...
test("a record stamped shipped leaves the container, whatever stands on disk", async () => {
  ...
  const recAbs = join(rec.path, `spec/iterations/${shipped}/record.md`);
...
test("a closed record's folder stays in the working tree", async () => {
  ...
  const dirRel = `spec/iterations/${id}`;
