...
test("/doc resolves wiki links to doclinks and leaves unresolved ones as text", async () => {
  ...
  const root = freshRoot();
  const session = new Session(root);
  writeFileSync(join(root, "linkdoc.md"), "see [[meth-morphological-analysis]] beside [[no-such-node]]\n", "utf8");
  const server = startMirror({ session, root, port: 0, log: new CallLog(seDir(root)), mode: "agent" });
  await new Promise((r) => server.on("listening", r));
  const port = (server.address() as { port: number }).port;
  try {
    const d = (await (await fetch(`http://127.0.0.1:${port}/doc?path=${encodeURIComponent("linkdoc.md")}`)).json()) as {
      html: string;
    };
    assert.match(
      d.html,
      /<a class="doclink" data-path="deliverable\/machines\/methods\/meth-morphological-analysis\.md">meth-morphological-analysis<\/a>/,
