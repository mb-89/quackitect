...
test("the close leaves the root tree clean, because it merges nothing", () => {
  ...
  mkdirSync(join(root, "spec", "expeditions", e.id), { recursive: true });
  writeFileSync(
    join(root, "spec", "expeditions", e.id, "report.md"),
...
test("closing on an unconfirmed report is refused, and the override is recorded", () => {
  ...
  const rec = readFileSync(join(root, "spec", "expeditions", a.id, "record.md"), "utf8");
...
test("a record that will not parse is marked, and the container still stands", () => {
  ...
  const rec = join(bad.path, "spec", "expeditions", bad.id, "record.md");
...
test("the close COMMITS the trunk's strays rather than refusing, and says which", () => {
  ...
  mkdirSync(join(e.path, "spec", "expeditions", e.id), { recursive: true });
  writeFileSync(
    join(e.path, "spec", "expeditions", e.id, "report.md"),
    "---\nform: expedition-leave\nstatus: done\nby: human\n---\n\nprobe report\n",
  );
...
test("a broken sub-canvas refuses typed at entry; fixing it heals on the next tick", async () => {
  ...
  const p = join(root, "deliverable", "machines", "ideation.canvas");
