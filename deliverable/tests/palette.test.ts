...
//
// The values are read from deliverable/brand/palette.css, so this lints the FILE a
...
test("no six-digit hex is written into the renderer", () => {
  ...
  assert.deepEqual(offenders, [], "colours belong in deliverable/brand/palette.css");
});
