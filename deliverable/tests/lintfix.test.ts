...
process.env.SE_BIOME_JS = join(REPO_ROOT, "deliverable", "node_modules", "@biomejs", "biome", "bin", "biome");
...
function lintedRoot(): string {
  ...
  const del = join(root, "deliverable");
...
test("a write to a covered file comes back formatted, announced, hash refreshed", () => {
  ...
  const r = fileWrite(root, "deliverable/src/messy.ts", "export const  answer=41+ 1\n", null);
  const disk = readFileSync(join(root, "deliverable", "src", "messy.ts"), "utf8");
...
test("a finding the safe fixes cannot reach rides the result", () => {
  const root = lintedRoot();
  const r = fileWrite(root, "deliverable/src/loose.ts", "export function f(x: any): any {\n  return x;\n}\n", null);
...
test("an uncovered file is left exactly as written", () => {
  const root = lintedRoot();
  const raw = "const   x=1\n";
  const r = fileWrite(root, "tools/outside.ts", raw, null);
  assert.equal(readFileSync(join(root, "tools", "outside.ts"), "utf8"), raw, "outside the config, the write stands verbatim");
