...
function mint(root: string, id: string, overrides: Record<string, string>, body = ""): void {
  ...
  lines.push("---", "", body);
  const dir = join(root, "spec/trace/requirement");
...
function mintSpec(root: string, id: string, method: string, verifies: string[], files: string[], extra: string[] = []): void {
  const dir = join(root, "spec/trace/test-spec");
  mkdirSync(dir, { recursive: true });
...
function mintTyped(root: string, folder: string, id: string, type: string, extra: string[] = []): void {
  const dir = join(root, `spec/trace/${folder}`);
  mkdirSync(dir, { recursive: true });
  writeFileSync(
    join(dir, `${id}.md`),
...
describe("the test-spec law", { concurrency: true }, () => {
  test("a matching spec covers its requirement, and the law stands silent", () => {
    ...
    writeFileSync(join(root, "deliverable/tests/x.test.ts"), "// a case\n", "utf8");
...
describe("the design-spec law", { concurrency: true }, () => {
  test("a spec realizes its element, and the law stands silent", () => {
    ...
    mintTyped(root, "design-spec", "dsp-a", "design-spec", ["realizes:", "  - el-a", "files:", "  - deliverable/engine/a.ts"]);
  ...
  test("a promoted experiment is assigned to a step of the drawing, or named", () => {
    const root = freshRoot();
    mkdirSync(join(root, "spec/iterations/itx/machines"), { recursive: true });
    writeFileSync(
      join(root, "spec/iterations/itx/machines/build-chunks.md"),
  ...
  test("a standing experiment from another record stays out of the assignment sweep", () => {
    ...
    mkdirSync(join(root, "spec/iterations/itx/evidence"), { recursive: true });
    writeFileSync(
      join(root, "spec/iterations/itx/evidence/fold-back.md"),
  ...
  test("a second record's fold-back is not read as this record's own", () => {
    ...
    const root = freshRoot();
    mkdirSync(join(root, "spec/iterations/itx-here/machines"), { recursive: true });
    writeFileSync(
      join(root, "spec/iterations/itx-here/machines/build-chunks.md"),
      ["---", "steps:", "  - id: c1", '    statement: "the first chunk"', "---", ""].join("\n"),
      "utf8",
    );
    mkdirSync(join(root, "spec/iterations/ity-there/evidence"), { recursive: true });
    writeFileSync(
      join(root, "spec/iterations/ity-there/evidence/fold-back.md"),
  ...
  test("trace-design names a ghost file and sweeps the unclaimed", () => {
    ...
    writeFileSync(join(root, "deliverable/engine/real.ts"), "// code\n", "utf8");
    writeFileSync(join(root, "deliverable/engine/stray.ts"), "// nobody claims me\n", "utf8");
    mintTyped(root, "element", "el-a", "element");
    mintTyped(root, "design-spec", "dsp-a", "design-spec", [
      "realizes:",
      "  - el-a",
      "files:",
      "  - deliverable/engine/real.ts",
      "  - deliverable/engine/ghost.ts",
    ]);
    const p = traceDesignLawProblems(loadTrace(root), root).join(" | ");
    assert.match(p, /dsp-a: names deliverable\/engine\/ghost\.ts, which does not exist/);
