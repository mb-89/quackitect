...
function benchRepo(): { root: string; rewind: string; futureCommit: string } {
  ...
  gitInit(root);
  write(root, `spec/iterations/${SUBJECT}/record.md`, `---\nid: ${SUBJECT}\nstatus: shipped\n---\n`);
  ...
  write(root, `spec/iterations/${SUBJECT}/machines/seeded.json`, JSON.stringify({ change_size: "major" }));
  write(root, `spec/iterations/${OLDER}/record.md`, `---\nid: ${OLDER}\nstatus: shipped\n---\n`);
...
describe("a bound run cannot reach past its rewind point", { concurrency: true }, () => {
  ...
  test("standing the tree rewinds the spec and leaves the machine current", () => {
    ...
    assert.deepEqual(stood.rewound, ["spec"], "only the subject is rewound");
    assert.deepEqual(
      stood.current,
    ...
    assert.ok(
      !existsSync(join(into, "spec/trace/requirement/req-the-answer-the-benchmark-must-not-see.md")),
  ...
  test("the pool is what can BIND, and what it dropped is counted rather than swallowed", () => {
    ...
    const { root } = benchRepo();
    write(root, "spec/iterations/i88-shipped-but-never-started/record.md", "---\nid: i88\nstatus: shipped\n---\n");
...
test("a stamp_covers line that names a directory with no hash is refused", () => {
  ...
  const r = fullReport();
  r.stamp_covers = "guidance= deliverable/engine=abc123";
  const problems = reportProblems(r);
  assert.equal(problems.length, 1);
  assert.match(problems[0], /guidance/, "and it names the directory that carries no hash");
});
...
describe("the benchmark reports are concealed while a run is bound", { concurrency: true }, () => {
  const REPORT = "spec/benchmarks/bench-i33.md";

  function boundRoot(): string {
    const root = freshRoot();
    write(root, REPORT, "iteration: i33\nran_at: 2026-01-01\n");
    write(root, "spec/trace/requirement/req-plain.md", "nothing to hide\n");
  ...
  test("a path that merely RESEMBLES a reports path is never concealed", () => {
    ...
    assert.equal(concealedFromLane("spec/benchmarksomething/notes.md", true), false);
    assert.equal(concealedFromLane("elsewhere/spec/benchmarks/x.md", true), true);
  });
  ...
  test("a glob over the reports folder returns nothing while bound", () => {
    const root = boundRoot();
    assert.deepEqual(fileGlob(root, "spec/benchmarks/**").files, []);
  });

  test("a listing does not name the reports folder while bound", () => {
    const root = boundRoot();
    const names = fileList(root, "spec").entries.map((e) => e.name);
  ...
  test("with no run bound every one of those four answers normally", () => {
    ...
    assert.ok(fileGlob(root, "spec/benchmarks/**").files.length > 0, "the glob finds it");
    assert.ok(
      fileList(root, "spec")
