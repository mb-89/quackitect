...
test("a file-ref field demands the named file on disk", () => {
  const root = freshRoot();
  const tpl = join(root, "deliverable", "machines", "forms", "templates");
  ...
  assert.match(claimProblems(root, state, body("dist/missing.zip"), []).join(" "), /no file on disk at/);

  mkdirSync(join(root, "dist"), { recursive: true });
  writeFileSync(join(root, "dist", "built.zip"), "zip", "utf8");
  assert.deepEqual(claimProblems(root, state, body("dist/built.zip"), []), []);
