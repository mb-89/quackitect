...
describe("what the controls write", () => {
  function base(): { root: string; read: () => Record<string, unknown> } {
    ...
    writeFileSync(join(root, "v.base"), "views:\n  - type: table\n    name: V\n");
    return { root, read: () => parse(readFileSync(join(root, "v.base"), "utf8")) as Record<string, unknown> };
  }
  ...
  test("what setGroupBy wrote is what the renderer reads back", () => {
    ...
    const spec2 = parseBase(readFileSync(join(b.root, "v.base"), "utf8"));
