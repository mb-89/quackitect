...
// `deliverable/machines/methods/meth-cockburn-use-case.md` — compiles
...
describe("the check has teeth", { concurrency: true }, () => {
  ...
  test("it rejects a well-shaped path with nothing behind it", () => {
    assert.equal(readable("deliverable/machines/methods/meth-does-not-exist.md"), false);
  });

  test("it accepts the document that row actually meant", () => {
    assert.equal(readable("deliverable/machines/methods/meth-cockburn-use-case.md"), true);
  });
});
