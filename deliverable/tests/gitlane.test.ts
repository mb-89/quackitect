...
test("the dirty gate excuses the engine's own trail, and nothing else", () => {
  const trail = " M spec/expeditions/e31-x/decisions.jsonl";
  assert.deepEqual(dirtyLines(trail), [], "the engine's own trail is not somebody's uncommitted work");
  assert.deepEqual(dirtyLines("?? spec/iterations/i1-y/decisions.jsonl"), [], "an iteration's trail likewise");
  const real = `${trail}\n M ${anyGuidanceDoc()}`;
  ...
  assert.deepEqual(dirtyLines("?? .mcp.json\n?? .claude/"), [], "untracked files never block a reconcile");
