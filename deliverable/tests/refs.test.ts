...
describe("typed references", { concurrency: true }, () => {
  ...
  test("a resolved node that does not answer its own template is refused", () => {
    const root = corpus();
    const dir = join(root, "spec", "trace", "value-prop");
  ...
  test("the same path with backslashes resolves — that is what Windows copies", () => {
    assert.deepEqual(check(corpus(), "value-prop", "- spec\\trace\\value-prop\\vp-a.md"), []);
  });
  ...
  test("generosity about SHAPE is not generosity about existence", () => {
    assert.match(check(corpus(), "value-prop", "- spec/trace/value-prop/vp-nowhere.md")[0] ?? "", /no artifact for/);
    assert.match(check(corpus(), "value-prop", "- spec/trace/stakeholder/stk-a.md").join(" "), /every reference is a value-prop/);
  });
  ...
  test("the placeholder is expanded from the FIELD's type, never copied", () => {
    const root = corpus();
    const dir = join(root, "deliverable", "machines", "items");
    mkdirSync(dir, { recursive: true });
    ...
    assert.equal(
      h.placeholder,
      "path from the project root, e.g. spec/trace/neighbour/nbr-something.md",
      "it SAYS where the path is measured from, and shows one",
    );
    assert.equal(h.description, "one neighbour REFERENCE per line");
    assert.equal(h.of_template, "deliverable/machines/items/neighbour.md", "the reader is one click from the rules");
  });
  ...
  test("a field with no declared type still reads, and links nowhere", () => {
    ...
    assert.equal(h.placeholder, "e.g. spec/trace/something.md", "still a path, and it still starts project/");
