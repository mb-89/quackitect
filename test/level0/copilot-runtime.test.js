...
function fixture() {
  ...
  const proc = fakeProc({
    ...
    "/tree/.se/.runtime/bin/vale"(_argv, init) {
    ...
    "/tree/.se/.runtime/bin/biome"(argv) {
...
test("a hold standing drops its step's note from the layer the runtime hands", async () => {
  ...
  it.disk.write("/tree/.se/.runtime/box.json", JSON.stringify({ id: "d462e994b4cef" }));
  it.disk.write(
    "/tree/.se/.runtime/hold/box-d462e994b4cef-claude-code.json",
    JSON.stringify({
      ticket: "a-child",
      step: "design/draft",
