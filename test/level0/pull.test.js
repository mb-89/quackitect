...
test("on trunk a cloud box's pull takes a branch, and a desk's pull takes none", () => {
  ...
  const { code, said } = heard(() => pulling(ROOT, ["pull"], desk.it));
...
test("on trunk a desk's pull hands out no group and no group's child, and cuts a branch for an open group standing without one", () => {
  ...
  const again = heard(() => pulling(ROOT, ["pull"], stands.it));
...
test("on trunk the drop and the hold come before the take, so a held hand takes no branch", () => {
  ...
  const held = heard(() => pulling(ROOT, ["pull"], it));
  ...
  const dropped = heard(() => pulling(ROOT, ["pull", "--drop"], it));
...
test("a hand takes a leaf it passed back, and another hand's leaf stays", () => {
  ...
  const refused = heard(() =>
    pulling(ROOT, ["pull", "a-child", "--back", "design/draft"], other.it),
  );
...
test("a hold drops on request, and the leaf stays where it stands", () => {
  ...
  const { code, said } = heard(() => pulling(ROOT, ["pull", "--drop"], it));
...
test("a hand under --as works one step under its own name, and the pull answers done after it", () => {
  ...
  const out = heard(() => pulling(ROOT, ["pull", "--as", "helper-2"], it));
  ...
  const back = heard(() => pulling(ROOT, ["pull", "a-child", "--as", "helper-2"], it));
...
// A bare name on a leaf with no verdict field shows the leaf, and lands nothing. [[spec/design_output/pull#the-bare-pull-shows-the-leaf]]
