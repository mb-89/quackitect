...
test("a revert over a pull commit refuses, and names the take-back verb with the leaf", () => {
  ...
  assert.match(said[0].message, /\.\/RUNME\.sh ticket pull a-child --back design\/draft/);
...
test("a revert over a plain commit passes, and a bare reset or one over paths reads nothing", () => {
  ...
  for (const command of ["git reset", "git reset --hard", "git reset -- a.js", "git reset HEAD a.js"]) {
...
test("the leaf reads off each subject form, and a subject naming none says <leaf>", () => {
  for (const [subject, leaf] of [
    ...
    ["a-child: passes implement/tests-red, skips implement/reflect", "implement/tests-red"],
    ["a-child: opens", "<leaf>"],
  ]) {
...
function box() {
  return {
    env: {},
    disk: fakeDisk({ [`${ROOT}/spec/tickets/a-child.md`]: "# Ask\n" }),
    proc: fakeProc({
...
test("the door reads a reverted commit alone, and refuses a pull commit it takes back", async () => {
  assert.equal(denied(await onBash({ command: "git revert abc123" }, box())), "");
  assert.match(
    denied(await onBash({ command: "git revert def456" }, box())),
...
test("the door refuses a reset over a pull commit, and passes one over plain commits", async () => {
  assert.match(
    denied(await onBash({ command: "git reset --hard HEAD~2" }, box())),
    /PullCommitStands/,
  );
  assert.equal(denied(await onBash({ command: "git reset --hard HEAD~1" }, box())), "");
});
