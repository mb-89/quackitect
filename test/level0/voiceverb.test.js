...
const ROOT = "/tree";
const BIN = "/tree/.se/.runtime/bin/vale";
...
test("a fixture transcript yields its answers as files", async () => {
  ...
  const proc = fakeProc({ [valeArgv(".se/.runtime/measure")]: { stdout: "{}" } });
  ...
  assert.equal(out.code, 0);
  assert.equal(
    disk.read(`${ROOT}/.se/.runtime/measure/sess/001-answer.md`),
    `${TEN} ${TEN} ${TEN}\n`,
    "the answer lands under a name the answer register reads",
  );
  assert.equal(
    disk.exists(`${ROOT}/.se/.runtime/measure/sess/002-answer.md`),
    false,
    "the sidechain and the user turn write nothing",
  );
  assert.match(out.text, /^1 answer\(s\) under \.se\/\.runtime\/measure\.$/m);
