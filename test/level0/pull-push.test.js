...
const FREE = (step = "design/draft") =>
  filled(
    filled(CHILD("open", step).replace("group: one-group\n", ""), "### approach", "The approach."),
    "### tests",
...
// [[spec/design_output/pull#the-rejected-push]]
test("on trunk a pass runs the check, then the push, in that order", () => {
  const { code, said, ran } = handedBack("design/draft");
  assert.equal(code, 0, said);
  const checked = ran.indexOf(CHECK);
  assert.ok(checked >= 0, "the check runs over the close");
  assert.ok(ran.indexOf(PUSH) > checked, "the push goes after the check");
  assert.ok(checked > ran.indexOf("git commit -m free-one: passes design/draft"));
  assert.ok(!said.includes("moves"), "no moved-branch sentence stands");
});

// [[spec/design_output/pull#the-rejected-push]]
test("a push the door refuses prints the door's own line, and no rebase runs", () => {
  const { code, said, ran } = handedBack("design/draft", {
    [PUSH]: {
      exitCode: 1,
      stderr:
  ...
  assert.match(said, /main takes a green battery, and the stamp names another commit\./);
  assert.ok(!ran.some((one) => one.startsWith("git rebase")), "no rebase runs");
  assert.ok(!said.includes("moves"), "the answer names no moved branch");
});
...
test("on trunk a rebase moves the commit off the stamp, so the check runs again before the second push", () => {
  ...
  assert.ok(ran.lastIndexOf(CHECK) > rebase, "the second check reads the rebased commit");
  assert.ok(ran.lastIndexOf(PUSH) > ran.lastIndexOf(CHECK));
});

// [[spec/design_output/pull#the-rejected-push]]
test("a tests-red close on trunk runs no push, and the answer says it stands local", () => {
  ...
  assert.equal(code, 0, said);
  assert.equal(fieldOf(disk.read(at("spec/tickets/free-one.md")), "step"), "implement/change");
  assert.ok(ran.includes("git commit -m free-one: passes implement/tests-red, skips implement/reflect"));
  assert.ok(!ran.includes(PUSH), "the close stays local");
  assert.ok(!ran.includes(CHECK), "a red close runs no check");
  assert.match(said, /stands on this box, and the next green push carries it/);
});

// [[spec/design_output/pull#the-rejected-push]]
test("a red check on trunk past tests-red keeps the close local, and the answer names the check's cause", () => {
  const red = { exitCode: 1, stderr: "EveryModuleTested: src/scripts/x.js has no test\n" };
  const { code, said, ran } = handedBack("design/draft", { [CHECK]: red });
  assert.equal(code, 1);
  assert.match(said, /^refused/m);
  assert.match(said, /check answers red/);
  assert.ok(said.includes(saidBy(red)), "the answer names the check's own cause");
...
test("a hand-back the record holds already pushes again, and a refusal says the record holds it", () => {
  ...
  const { it, disk } = doors(standing(answered, withField(GROUP_NOTE, "step", "children")), {
