...
test("a stub holds its files, reads every one back, and no file of the method", () => {
  ...
  try {
    ...
    assert.deepEqual(
      walk(dest),
      [...said.files].sort(),
      "every file the list names, and nothing else",
    );
    for (const one of said.files)
...
test("a stub's plugin carries the name its settings allow, so a tool answers to it", () => {
  ...
  try {
    ...
    assert.equal(
      JSON.parse(files.read(join(dest, MARKER))).name,
      name,
      "the manifest names the folder",
    );
    const skills = files.list(join(dest, ".claude", "skills")).map((one) => one.name);
    assert.deepEqual(
      skills,
      [name],
      "the stub carries the one plugin, under that name",
    );
    const allow = JSON.parse(files.read(join(dest, ".claude", "settings.json")))
      .permissions.allow;
...
test("the shim finds the vehicle through the register, and hands argv and the work root on", () => {
  ...
  try {
    ...
    files.write(
      ...
      JSON.stringify([
        { id: "abc123", version: "0", method_root: vehicle, registered: "now" },
      ]),
    );
    files.makeDir(dest);
    files.write(
      ...
      JSON.stringify({
        vehicle: "abc123",
        name: "acme",
        upstream: "https://host/a/b.git",
      }),
    );
    files.write(
      join(dest, "RUNME.sh"),
      files.read(join(root, "src", "stub", "RUNME.sh")),
    );
  ...
  async () => {
    ...
    try {
      ...
      const $ = {
        ...
        process: {
          run: async (argv, init = {}) =>
            outside.run(argv, { ...init, cwd: dest, env }),
        },
      ...
      assert.ok(
        files.exists(join(cloned, "RUNME.sh")),
        `the upstream stands at ${cloned}: ${logged.join(" ")}`,
      );
      ...
      const pointer = JSON.parse(
        files.read(join(dest, ".se", ".runtime", "vehicle.json")),
      );
      ...
      assert.equal(
        entry.port,
        pointer.port,
        "the register holds the clone at the pointer's port",
      );
      ...
      assert.match(
        shim.stdout,
        new RegExp(`method\\s+${either(cloned)}`),
        "the shim finds the clone",
      );
    } finally {
      files.remove(where);
    }
