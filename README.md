...
editor. On Windows a person types `.\RUNME.ps1`, which hands the same words to
the shell Git ships, so one installer serves every box. `./RUNME.sh help` names
every verb.
...
1. Run `./RUNME.sh`, or `.\RUNME.ps1` on Windows.
