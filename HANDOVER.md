...
The branch carries every commit the pull writes, and the remote holds each one.
`./RUNME.sh check` answers 0 with the server standing.

| the ticket this box closes | how |
|---|---|
| a-base-file-says-it | its evidence command reads `branch test`, which answers `green` |
| a-cell-takes-an-edit | the same |
| the-help-reads-the-cursor | the same |
| the-tree-draws-its-columns | the same |
| the-tree-takes-a-filter | the same |
