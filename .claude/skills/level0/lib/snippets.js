...
// Vale hands a rule the whole file, frontmatter and all. A verb writes those fields and the schema rules them, so a prose rule blanks that block and keeps every offset true. [[spec/design_output/projection#a-layer-writes-two-files]]
export const FRONT = "said := blanked(scope, `(?s)^---\\n.*?\\n---`)";
...
export function prelude(wanted, blanks = true) {
