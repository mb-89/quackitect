...
export function register(on, _options) {
  ...
  on("tool.call", { tool: STOP_CALL }, async ($, e, _next) => {
    ...
    // [[spec/design_output/stop#the-mark-survives-a-reload]]
    if (firstTurn && (await turnedMark($))) {
      firstTurn = false;
      await logbook.say("debug", "level0", "the module runs again past turn one", { file: TURNED });
    }
  ...
  on("turn.complete", async ($, e, next) => {
    ...
    if (firstTurn && e.reason === "answer") {
      firstTurn = false;
      await markTurned($, new Date().toISOString());
...
// The client reloads the module when one of its files changes, and a fresh module starts at turn one. The mark on disk says turn one is past. [[spec/design_output/stop#the-mark-survives-a-reload]]
const TURNED = ".se/level0.turned";

async function turnedMark($) {
  try {
    return Boolean(String(await $.fs.read(TURNED)).trim());
  } catch {
    return false;
  }
}

async function markTurned($, at) {
  try {
    await $.fs.write(TURNED, `${at}\n`);
  } catch {}
}
