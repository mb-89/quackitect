...
#
#   sh util/checks/benchmark.sh            the engine's benchmarks, then the battery's lanes
#   sh util/checks/benchmark.sh -count 3   fewer runs of the Go benchmarks
#   sh util/checks/benchmark.sh --rank <page>   rank a battery page already written
set -e
...
if [ -z "$page" ]; then
  ...
  sh util/checks/battery.sh > "$page" 2>&1 || true
fi
