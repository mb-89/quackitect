...
4. Carry on to the end of the work, and never wait. While a helper or a test runs, take the next todo, the next ticket or a problem in the Problems panel. A finished piece opens the next one, and takes one `mcp__level0__report` line. Land each helper's work through `./RUNME.sh commit` once its report and tests pass. One review then reads it, and one undo takes it back. *
