...
8. Green the check before you hand the branch back, whatever hand puts the fault there. A fault you inherit is no fault of yours, and a fault you hand on is. *
9. Write your result, your retro and every script you write under `.se/scripts` into `HANDOVER.md`, replacing the brief.
10. Run `./RUNME.sh branch done` last. It sets the status and pushes. The next box reads that status, and it merges nothing a box still holds.
11. Stop at the push. The merge into `main` and the branch's delete happen off this box.
12. Stop after `branch done`. One session works one branch, and the next branch belongs to the next session. *
13. Say in your answer which commit you are on, and whether it matches origin. *
