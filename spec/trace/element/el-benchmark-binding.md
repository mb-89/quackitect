...
- Exports `spec` at that commit into a fresh directory.
...
It never touches `deliverable`. The engine measured is the current one
([[raid-dec-a-benchmark-rewinds-content-and-never-the-machine]]).
...
`deliverable` and `guidance` come from TODAY. A whole-tree
