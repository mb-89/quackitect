...
realizes:
  ...
  - el-front-desk
files:
  - deliverable/engine/pool.ts
  - deliverable/engine/inbox.ts
...
`spec/trace/option/opt-<slug>.md`, a corpus node like any other.
...
`spec` recursively, so a minted option is swept for leaked names and
