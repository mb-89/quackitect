...
WHAT IT DELIBERATELY DOES NOT DO. It never touches `deliverable`. The
...
THE REWIND IS THREE-WAY, NOT TWO-WAY. `spec` rewound; `deliverable`
and `guidance` current; history bounded. A whole-tree rewind FAILS TO
COMPILE, and the engine proved that itself rather than a reviewer noticing.

`guidance` IS METHOD RATHER THAN SUBJECT. That is why it stays current,
