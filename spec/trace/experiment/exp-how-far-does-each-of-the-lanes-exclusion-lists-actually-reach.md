...
| `.obsidian` | reaches | **7 entries** | **12** | **9214 matches** |
| `.github` | reaches | **1 entry** | **1** | **670 matches** |

### And a fourth shape nobody had named

A DIRECTORY EXCLUDED FROM A PARENT WALK IS STILL LISTABLE WHEN NAMED. Listing
the root hides `.se`, `.git` and `node_modules`. Listing `.obsidian`
...
needs `spec/benchmarks` invisible to read, list, glob and search alike,
