...
## Closed at rank-unknowns, 2026-08-20 — the two readings now agree

THIS ENTRY SAYS "under any ordering that respects those grades the leader loses".
THAT IS NO LONGER TRUE and it stopped being true when two axes were re-scored.

WEIGHTED BY THE AXES' OWN `breaks_how_badly` — fatal 4, crippling 3, corrosive 2,
read off each requirement rather than assumed:
`cand-whoever-holds-the-hands-decides` 36, `cand-the-reader-beside-the-walk` 33,
`cand-the-seed-made-total` 26, `cand-the-derived-ladder` 26.

UNWEIGHTED: 12, 11, 9, 9. SAME ORDER.

SO THE MODELLING CHOICE THIS ENTRY WORRIED ABOUT NO LONGER DECIDES ANYTHING. The
risk was real when it was written — the two readings then picked different
winners — and the thing that resolved it was not a decision about weighting. It
was a scorer correcting an anchor that quoted a candidate's false sentence about
itself.

AND THE LINE THAT LED UNDER THE GRADES IS ELIMINATED ANYWAY.
`cand-the-reader-beside-the-walk` violates two musts, so it is not a candidate
under either reading.

WHAT THE ENTRY GOT RIGHT AND IS WORTH KEEPING: refusing to weight is not a
neutral default. It discards information the axes carry. That argument stands and
is now recorded in `converge-pugh`, where both readings are reported side by
side rather than one being chosen.
