...
type: "[[raid]]"
kind: issue
statement: se-start.ts and se-arrive.ts each place .mcp.json and .claude/settings.json from the same templates, and nothing checks that the two agree.
owner: the owner
...
any edit to `deliverable/cage/` has to be made twice and checked by
hand.
