...
type: "[[raid]]"
kind: assumption
statement: The options pool is a node kind stored under spec/ like every other corpus node, so minting an option needs no store of its own.
...
THE CORPUS ALREADY STORES TYPED NODES with frontmatter under `spec/`,
