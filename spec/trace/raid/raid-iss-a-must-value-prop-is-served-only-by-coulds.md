...
type: "[[raid]]"
kind: issue
statement: The vendoring promise inside vp-the-engine is carried by a use case and four requirements graded should or could, so the grades disagree with the promise they serve.
owner: the owner
...
- vp-the-engine — `priority: must`, and it is the vendoring criterion inside it that these five carry
...
vp-the-engine's own reason for being a must is recorded on the node: quackitect
...
## Amended 2026-08-21, when vendoring folded into the engine

THE SUBJECT MOVED AND THE ISSUE DID NOT. This entry was minted against
`vp-vendoring`, a proposition of its own. At the owner's ruling that
proposition folded into [[vp-the-engine]] as a criterion, and its audience
folded into [[stk-engineer-driving-agents]].

WHAT IS UNCHANGED IS THE COMPLAINT. Five nodes carry a must-grade promise at
should or could, and a priority cut reading grades alone would still take the
mechanism and leave the promise standing.

WHAT DID CHANGE IS THE READING. The parent is no longer a proposition served
only by these five, so the grade gap is now between a criterion and its
carriers rather than between a node and all of its children. That is a
