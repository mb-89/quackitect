...
- 6e. THE READER DELEGATES AND THE RECORD REFUSES TO HEAR IT. Registering a spawned hand rides one verb, and that verb is not legal in every state, so the registration is refused exactly where a hand was just spawned. A hand that was started is a fact about the world, and refusing to record a fact makes the account wrong rather than the hand unspawned. The registration must be accepted wherever the walk stands. Only the registration is widened; nothing else that verb carries moves with it. Registered: `raid-risk-widening-a-verb-s-legality-weakens-the-state-gate`.
