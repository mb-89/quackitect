...
style of `deliverable/machines/panels/controls.md`.
...
`deliverable/engine/tables.ts` states the rule at line 6: "The format is
the owner's, not ours; we render it."

### What is already built, measured 2026-08-01

- `deliverable/engine/tables.ts` — 479 lines. It reads `.base` and
  renders `type: table` and `type: pivot`.
- `deliverable/tests/tables.test.ts` — 184 lines, 20 tests, all passing.
- `deliverable/engine/render.ts` — imports the table widget at line 25.
...
`deliverable/tests/fixtures/rigor-matrix.base` declares fourteen
...
Save as `deliverable/machines/states/states.base`.
...
1. In `deliverable/tests/fixtures/depends.base`, change
   `rows: file.name` to `rows: name`. The matrix gains a diagonal. Measured: 48
   shared labels instead of 0.
2. In `deliverable/engine/tables.ts`, add an optional `order` to a pivot
