---
kind: [[rationale]]
explains: [[spec/guidance/voice-checks]]
---

# Why

A rule a hand reads at the turn's start stands far from the sentence that breaks
it. The voice rules bind every answer, and a hand holding all fifteen in mind
still writes the sentence one of them refuses.

This tree measured that in one session:

| what the hand does | what it carries |
|---|---|
| writes the single point of truth rule into two notes | the rule, an hour old |
| grounds two drafts on a claim about a folder | the rule, and two reviews refusing both |

So the checks read as questions, and a question asked of a finished sentence
lands where a rule read at the start does not.

The output style carries the actionables of a note marked `style: true`, and the
guidance schema holds a note to fifteen items. The voice note stands full, so a
check joining it pushes a rule out. A note of its own costs one section in the
style, and the style reaches a hand at every turn.

## 1. The reader acts on it

A sentence a reader acts on nowhere still costs them the reading. It also costs
whoever keeps it true. The check sorts the two, and a hand answering it cuts
more than it keeps.

## 2. Inside your authority

An aside about another's mechanism ages where nobody looks. A hand changing that
mechanism reads the notes it owns, and reads no note that mentioned it in
passing. So the aside stands wrong, and the hand who wrote it moved on.

## 3. No assertion from recall

A claim from recall reads right to the hand writing it. A store the hand reads
is a source, and this check leaves it alone. This tree refused two such claims
in one session, and one grep answered each. So the cost of checking stands under
the cost of a review.

## 4. No number in prose

A number lands stale. One draft wrote the count of files naming a tool, and the
draft's own file joined that list as it landed. A command stays true, and a
rationale holds the number an argument needs.

A date fixing a source is the one number that holds. The author and the year are
what let a reader find the source and check the claim on it. For details, see
[[spec/rationales/arguing]].

## 5. It stands nowhere already

A second wording drifts faster than a second copy, because a search finds a copy
and misses a wording. The check asks before the words, where the finding costs
one search. For details, see [[spec/rationales/working]].
