...
v3 could record which vehicle drove a project, and resolving one always failed. It
...
|---|---|
| an identity made at install | the vehicle moving |
...
anything. A stray vehicle under the per-user data folder made every program under
...
| name the vehicle | the install makes its identity |
| pick a driver | one vehicle on the machine answers itself |
