...
**Round 1 — Verify.** Every axis carries at least two elaborated alternatives (nine axes, 20 candidates total); each rejected option names its reason; the criteria table derives every weight from named requirements; feasibility names its two spike targets instead of hand-waving them.
