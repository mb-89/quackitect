...
TL;DR: The first returned book copy came back with 45 comments. They name real reader pain: internals leak into tables, queries render poorly or empty, navigation scrolls instead of paging, and the prose is dry. i0014 works every kept comment into the template and the spec, in sync.
...
4. The rejected comments (c16, c28, c45 - text-less artifacts) stay rejected with that reason on record.

## Milestone review  -> i14-m1-gate

**Verify.** The field notes exist in the backlog (19 files, NOTE-20260707-1907*/1908*), each carrying its comment ids; the read-back ran against the returned copy; the triage verdicts are recorded in the notes.
