...
by: agent
signed_off: 2026-08-26T10:34:16.839Z
authors: agent
files:
---
...
Eight rules against ten clusters is eighty cells, and the pass is mechanical rather than clever. Every cell is written, including the ones that say nothing.

THE WORK CLUSTER BITES ON ALL EIGHT, which is unsurprising and still worth recording: it is the cluster under design, so every structuring rule has something to say about it. The other nine clusters produced three bites between them.
...
| --- | --- | --- |
| Group what changes together; separate what changes apart. | cluster-the-work | the five acts change together whenever the shape of a piece of work changes, which is why they cluster; it also says the holding pen changes with them |
| Group what changes together; separate what changes apart. | cluster-the-holding-pen | the same finding from the other side: a stray changes when work changes, so the pen belongs with the work |
...
| Make the common case cheap; make the rare case possible. | cluster-the-work | most work waits on nothing, so offer everything open and compute readiness only where a dependency was written down |
...
| One source of truth; everything else derives. | cluster-the-work | the count and the offer are views over the work and are never kept beside it, which is the corpus rule taken into this cluster |
| One source of truth; everything else derives. | cluster-the-account | the account reads what the work says rather than keeping a tally of its own |
...
| One source of truth; everything else derives. | cluster-the-walk | nothing |
| Push decisions to the last responsible moment. | cluster-the-work | derive an work token when something asks for it rather than the instant a position opens |
...
| Make the illegal unrepresentable, not merely checked. | cluster-the-work | a position with work still out should be unable to read as left, rather than checked and refused at the exit |
...
| Make the illegal unrepresentable, not merely checked. | cluster-the-walk | nothing |
| Small interfaces between big parts beat the reverse. | cluster-the-work | this cluster touches three others, and each contact should be one flow rather than several: the count outward, the settled result outward, the position inward |
...
| If it must be remembered, it must be recorded. | cluster-the-work | a raised mark and a close that is not done both have to be written on the work, because the next hand has no memory of either |
...
| The default should be the safe thing. | cluster-the-work | re-entering an unfinished position keeps its work by default, because remaking it silently loses work somebody already did |
...
| The default should be the safe thing. | cluster-the-walk | nothing |
...
SEVEN OF THE EIGHT BITES WERE ALREADY ON THE CHART, and only one produced a new option. That is the catalogue working as intended rather than failing: a rule that confirms an option another finder already minted is evidence the option is not arbitrary.

WHICH RULE CONFIRMED WHICH. Last responsible moment confirms deriving lazily. One source of truth confirms the index and the derived count. Illegal-unrepresentable confirms the authorisation card. Group-what-changes-together confirms both the work cluster itself and the holding-pen absorption. If-it-must-be-remembered and safe-by-default both confirm demands the register already carries.

THE ONE NEW OPTION IS THE CHEAP COMMON CASE: everything open is offered unless a dependency was written down, so readiness costs per declaration rather than per position.

SMALL-INTERFACES PRODUCED A CONSTRAINT RATHER THAN AN OPTION, and it belongs to the architecture milestone. This cluster touches three others and each contact should stay one flow wide.
