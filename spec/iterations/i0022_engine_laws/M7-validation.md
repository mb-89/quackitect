# M7 — Validate & accept (evidence)

## meets the need  → i22-m7-meets-the-need
...
## killer use-cases demonstrated  → i22-m7-killer-use-cases
...
## consistency swept  → i22-m7-consistency-swept

Everything this iteration changed is re-documented where it is described:

- implementation.md: the slot-end battery ritual is gone; the battery belongs to the gate, and the engine refusal is named.
- AGENTS.md: the `grant` verb, the killer-bless grant rule, the selftest channel note, and the recital preview clause.
- contract.md: the TL;DR card and preview mechanism (guarded by selftest:recital-chain); the provenance smear the sweep caught was removed.
- compose-reference.md: the voice-binds-nodes rule (landed at engage start).
- The retired-vocabulary sweep (test-prose-current) runs clean over all prompts and guides.
...
One gap, minted: [raid-mcp-arming](../../raid/raid-mcp-arming.md) - the MCP lane is
