...
Three experiments stand and each carries its own `folds_to` and `promote`. This form is the view over them; the answers live on the nodes.

### One promotion, two narrowings, and nothing reopened
...
`exp-two-hands-rating-the-same-six-cells` FOLDS TO `raid-risk-a-hand-declared-rung-drifts-upward-and-nothing-ever-says-so`, narrowed to rows that are one act and still open for drift over time, which two readers in one session cannot measure.

### The two absences are one absence, and it belongs to the build gate
...
THE ONE PROMOTION IS SMALL AND MECHANICAL and it goes to the build gate as a filter over the nodes rather than as a list here. A placeholder row has no single difficulty to declare, and three already exist — `M4_25 run-candidates`, `M6_15 run-spikes`, `M7_40 build-steps`.

WHAT THE BUILD SHOULD DO WITH IT, stated so the gate can rule rather than re-derive: either the loader refuses a complexity on a row whose work is delegated, or the complexity attaches to the seeded states. THE SECOND IS BETTER AND COSTS MORE, because the seeded states are drawn per record and their ratings would have to be drawn with them.

TWO REGISTER ENTRIES STAY OPEN ON PURPOSE. The dependency is confirmed, not closed — a confirmed dependency is still a dependency. The drift risk is narrowed, not closed — nothing here measured drift.

AND ONE ASSUMPTION UNDER EVERYTHING WAS NOT PROBED AND WILL NOT BE. Whether a stronger hand does better work on a harder step. Probe 3 tried at M4 and could not: the only paired walks in the corpus are forms and their own repairs. Every candidate leans on it and this iteration ships without it.
