...
1. **Parity standalone** — a generic `suite: standalone` test-node marker, skipped by tests-pass, own board entry ([adr-standalone-suite](../../decisions/adr-standalone-suite.md)). Not a name filter (not generic), not node retirement (history churn).
