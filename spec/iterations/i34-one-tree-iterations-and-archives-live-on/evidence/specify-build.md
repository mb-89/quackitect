...
form: specify-build
by: agent
signed_off: 2026-08-16T06:44:55.683Z
authors: agent
files: null
---
...
i34 stands at specify-build, the last state before observe-red.

THE CLAIM SYSTEM IS OUT OF THE TRACE. Twenty nodes deleted on the owner's ruling to remove it everywhere it ripples: six locking requirements plus four more that served the same mechanism, one use case, one story, three test-specs, one element, one function, one interface, four register entries and one design spec. Signed evidence keeps its references, as ruled.

THREE LIVE REQUIREMENTS WERE RE-POINTED rather than taken down with the use case — the container's DAG wiring and a record's shipped status have nothing to do with locking.

THE DESIGN SPECS STAND AT TWENTY-TWO. dsp-claim-lane went with its element; dsp-satellite-lifecycle dropped the interface that no longer exists.

NO SPEC IS AUTHORED HERE. i34 changes existing behaviour rather than adding an element, so the register is edited rather than extended — which is what minor means at this row.
...
- THE BUILD ORDER IS RISK FIRST, and the risk is unrecoverable loss rather than technical doubt. The rescue of the fourteen at-risk files runs before anything else, and the deletion runs last with its count re-measured at the moment it runs.
- THE SPINE IS ALREADY WALKED, which is why no spine-first pass is needed: the change map names every call site, and every one was opened this session rather than inferred.
- THREE DESIGN SPECS CARRY THE WORK. dsp-resolution-seam loses the tree-choosing half. dsp-record-lifecycle loses claims.ts and worktree.ts. dsp-walk-machine gains the selection state.
- NEXT IS observe-red, where the four new checks must fail before any old behaviour comes out. Three can fail today by test; the fourth is an inspection, and its red is a reading that names the choosers still standing.

## anything_else

NO PROMOTION ROWS, and the reason is that no spike ran. At minor the whole M6 prototype lane is dropped to none — rank-unknowns, run-spikes, fold-back and gate-prototype — so there is no promoted experiment to assign. An empty table is the honest answer rather than a gap.

WHAT THE STRATEGY LENSES SAY ABOUT THE ORDER, since the card asks for it rather than for a pick:

- RISK FIRST decides the shape. The only irreversible act in this iteration is a deletion, so the work that protects against it — the rescue — goes first, and the deletion goes last.
- PARALLEL FLOW says almost nothing here, and saying so is the answer. The nine steps are one deep chain: the open flag cannot change before the records are on trunk, and the worktrees cannot go before the flag stops reading them. Forcing width onto a chain only adds seams.
- THE SPINE IS NOT NEEDED because the seams are already known. Every call site is named with its file and line, read this session.
