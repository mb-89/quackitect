...
RENUMBERED AT THE MERGE, 2026-08-28. These two numbers are now `SE-C-154` and
`SE-C-155`. i63 shipped its own `SE-C-149` and `SE-C-150` on trunk first, and a
number in a shipped log is never reused. The observations below stand exactly as
they were made; only the numbers moved.
