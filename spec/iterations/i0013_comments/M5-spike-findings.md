...
Timeboxed spike: a probe script injected into a real book copy (spec/book.html, 370 KB), run in headless Chromium, verdicts read back mechanically. Spike home: data home `spikes/i13-annotator/` (throwaway). Four claims:
...
Not spiked: the File System Access API save (needs a user gesture — no headless path). Residual assumption, low risk (TiddlyWiki precedent, documented API); it is exercised live in the M7 killer-use-case demo.
...
Nothing needs a library, a service, or a new top-level command. Every piece lands in existing homes (book shell JS, emitter, note command, engine internals).

## Review rounds & verdict

1. **Verify.** The probe ran against the real artifact, not a mock; verdicts were extracted mechanically from the rendered DOM; the spike files sit in the data home for inspection until discarded.
2. **Validate.** The riskiest claims (paint-without-mutation, in-file anchoring, island confinement) are the ones the architecture stands on — all proven on the real book. The one failure (P3) produced a concrete build step instead of an M3 step-back: the architecture survives, the emitter grows one duty.
3. **Red-team.** Attack: "the save path — the Acrobat feel itself — was not spiked." Held: acknowledged openly as the residual assumption; it is API-documented, precedented, and demonstrated live at M7; a failure there degrades to the download fallback, which needs no API at all. Kill-criterion check from M1: the mark→save→read-back round-trip had to prove out — mark and island proved here, save carries a named fallback. No step-back triggered.
