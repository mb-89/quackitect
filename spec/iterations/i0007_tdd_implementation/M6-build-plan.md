...
7. **bs-research-ref** (#2) — research-as-pluggable reference (Claude Code: deep-research; inline elsewhere). Never vendored. → req-research-pluggable

Rollup `i7-m6-build` depends on the last step. Then follow:

- `designs-realized` (coverage)
- `internal-quality-ok`
- `verification-green` (coverage:tests-pass)
- `impl-risks-acceptable`
...
- `quack lint` → **coverage clean (no holes)**.
- `coverage:designs-realized` and `coverage:tests-pass` both **DONE** (all tests green across every iteration).
- `quack selftest` → **0 FAIL**.
