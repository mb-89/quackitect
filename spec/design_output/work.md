...
|---|---|---|
| release it | `branch release <name>` | writes `gave`, so the group stands at `todo` |
...
`branch done` on a group branch writes `gave` into the newest record entry, which
...
branch older than a rule passes `branch done` and reddens trunk at the merge.
