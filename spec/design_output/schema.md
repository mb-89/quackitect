...
A file under `spec/schemas` carrying no `body` describes no note, so the reader
passes over it. `paragraph.schema.yaml` is such a file: the projection reads it
and writes rules out of it, and no note ever names it.
