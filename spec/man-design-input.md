...
<!-- tailor: shipped text - the chapter OPENS with IFU prose, never a technical
  needs list (owner ruling, req-ch3-needs-intro): IFUs show what users can do,
  each tells a user story, the idea composes into needs; the flat index is the
  register, referenced, not repeated.
-->
<!-- ai:3 -->
The [IFUs](man-intro-ifus) show what users can do with the system. Every IFU tells a user story. Those stories compose the idea into needs, and the needs refine into every checkable claim below. The one flat index over all of them - needs, use cases, functions, requirements, qualities, constraints - is the [design input register](input-register) later in this chapter.
...
-->
<!-- ai:3 -->
How the rows interconnect lives in one place: the [trace](term:trace), one page per need, in the [overview chapter](man-overview.md). The register below is the one flat index over every input type - use cases and functions included, each traced to its need. Expand a row to read its definition; the type facet filters to any one kind:
...
<!-- ai:3 -->
Needs are the user level; requirements are the system level; deeper tiers hang off refines. Statements are [EARS](meth-ears)-shaped and carry their tolerances; no TBD survives the detail [gate](term:gate). The register's filter columns carry the facet coverage - a zero-count value is the completeness check, live. Click values to filter; several combine.
---
fig: input-register
---
...
-->
<!-- ai:3 -->
Where two stakeholders pull against each other, the tension renders below - the reasoning is one click away. Every requirement's source traces to a stakeholder row in the [orientation chapter](man-intro-ifus.md).
