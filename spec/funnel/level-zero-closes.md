...
applications, the specific processes.

Every piece level zero's close names stands built. For what each piece does,
see `./RUNME.sh help`, `./RUNME.sh doors` and the notes under
`spec/design_output`. For level one, see
[[spec/design_input/the-agent-pulls-tickets]]. This note keeps what the field
puts at this layer, and the questions that stay open.
