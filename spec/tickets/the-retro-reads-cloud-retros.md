...
- `retro collect` in `src/scripts/retro-collect.js` gathers the `retro` chapter of every group ticket that closes in the window, and a case in `test/level0/retro-collect.test.js` decides it
- `spec/guidance/retro/read.md` hands each of those chapters to the reader whose hours hold the close
- a cloud box's hand-back of a group keeps the session transcript where `retro collect` reads it once the branch leaves, and a case under `test/level0` decides it
