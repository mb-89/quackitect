...
The sidebar counts seven items, the work editor draws one, and a person trusts neither number. The count in the tab's name answers a third question, so matching it leaves the button wrong.

- one test lays the button's count beside the rows the tab draws on the same tree, and the two match. `go -C src/tui test ./...` decides it
