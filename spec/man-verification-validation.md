...
-->
<!-- ai:3 -->
Validation closes the V: the outcome stands against the needs of the [motivation chapter](man-motivation.md). The table below carries one row per need; expand a row for its pass lines. Acceptance state derives from the gates - the validation [milestone](term:milestone) carries the adjudication, and this chapter never restates it.
