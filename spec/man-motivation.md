...
<!-- ai:3 -->
When goals collide, trust wins. The [design input chapter](man-design-input.md) carries the tensions this forces.
...
<!-- ai:3 -->
The idea above composes into a set of needs quackitect has to serve. They are the top rows of the [design input register](man-design-input): each names the stakeholder it serves, and its expand carries the pass lines that accept it. The [validation chapter](man-verification-validation.md) traces each one to its outcome.
---
## Business case
<!-- fill [judgment]
