...
before it started, `spec/iterations/i33-.../record.md` stands with
...
THE REPORTS ARE THE SCHEDULER'S STATE. `spec/benchmarks/` says which
...
`deliverable/machines/items/` with `readdirSync`, so a new item
...
SO THAT MASK IS CONDITIONAL, NOT STATIC. `spec/benchmarks/` is hidden
