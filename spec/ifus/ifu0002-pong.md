...
mode: deck
kind: ifu
statement: a five-minutes walkthrough from an empty folder to a shipped Pong game.
arc:
  start: fresh
review-82079:
  completeness: every walk phase has its measured slide, the shipped game plays on the last story slide, and the coverage slide links the loop use cases the walk exercises
  correctness: the minutes are the measured 4:46 walk rounded to half-minute steps, and every install claim is backed by the probing scripts
  conciseness: one phase per slide with its real duration; the loop detail lives in the work-loop IFU and the method prompts
  comprehensibility: the arc runs from an empty folder to a playable game, and the timeline across the bottom keeps the pace honest
  minimalism: the deck restates no base state - it is the ruled fresh-start exception and says so on its first slide
  accessibility: rides the book shell's slide roles and labels; the onion and chat figures carry aria labels; the game is keyboard-driven
  target-group-fit: written for a newcomer proving the workflow in five minutes, before any commitment
---
...
|||
<!-- ai:3 -->
What to see on the right: pong's whole design input - one need, one use case, three requirements, each with its test. The engine computes the pairing; this book's own [design input register](input-register) is the same table at project scale.
|||
fig: sample-register
Minutes: 1.0
...
[uc-engage-start](uc-engage-start), [uc-engage-next](uc-engage-next), [uc-engage-ship](uc-engage-ship), [uc-lawful-walk](uc-lawful-walk).
Minutes: 0.0
