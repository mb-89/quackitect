...
The arrows are in `machineSvg` at deliverable/engine/render.ts:173-308. Real markers, real guard labels, drawn per edge:

- the line with its arrowhead, deliverable/engine/render.ts:207;
- the guard text at the midpoint, deliverable/engine/render.ts:209;
- the double border marking a sub-machine, deliverable/engine/render.ts:227-230.

### What version 1 did

Version 1 drew one machine figure, from one function: `svgModelGraph` in engine-go/models.go:325-390.

It had arrowheads and edge labels. It had no layout. A state machine is a single-layer graph, so it fell into the flat branch: a vertical column at x=400, ordered by sorted state identifier (engine-go/models.go:330 `sort.Strings(ids)`, placement at :358-363).
...
Version 2's board drew HTML boxes in a flexbox column and no edges at all. Between consecutive boxes it emitted a `▼` character (deliverable/bin/se-board.ts:438 and :452, at reference `v2`). That glyph means "and then", not "from A to B".

The edges existed in the model with roles and guard labels. The projection threw them away at the boundary: a frame's states carry no edges field (deliverable/engine/project.ts:56, reference `v2`).
...
The ranking function is real. `stateRows` at deliverable/engine/project.ts:65-80 (reference `v2`) computes a longest-path rank, breaks cycles by declaration order, and floors terminals last. I read all sixteen lines. They are correct.

The board never sorted by it. Two lines settle it, both at reference `v2`:

- deliverable/bin/se-board.ts:451 `while (i < f.states.length)` — the board walks the states array in its own order;
- deliverable/bin/se-board.ts:436 `let j = i + 1` — the side-by-side scan starts at the next neighbour and stops at the first mismatch.
...
- an ordered spline route through the states the walk passed, deliverable/engine/render.ts:278-279;
- a rotated "you are here" arrowhead, deliverable/engine/render.ts:299-302;
- a road-closure ring carrying its reason as a tooltip, deliverable/engine/render.ts:290-297.

It dropped automatic layout for drawn machines by explicit ruling. deliverable/engine/render.ts:92-103 reads: "THE DRAWING IS THE TRUTH, SIZE INCLUDED (owner ruling 2026-07-28) ... So the render now takes the geometry VERBATIM — position and size both. Fix it in Obsidian and it is fixed here."
...
Version 3 lays out every **generated** machine. The important one is `pinnedCanvas` at deliverable/engine/iterations.ts:562-583. It is a layered layout:

- each distinct milestone group becomes a column, deliverable/engine/iterations.ts:563-567;
- states stack downward inside their column, deliverable/engine/iterations.ts:573-575;
- coordinates are `x: col * 560, y: row * 260`, deliverable/engine/iterations.ts:576;
- edges are emitted for every transition, deliverable/engine/iterations.ts:580.

The milestone is the group. The compiler sets it at deliverable/engine/rigor-matrix.ts:270 `group: row.milestone`.
...
Two other generated layouts exist: the expedition machine (deliverable/engine/expmachine.ts:69-116) and the iterations container (deliverable/engine/iterations.ts:526-535). Both are hand-tuned with their own constants. Node box sizes come from one shared sizer, `nodeSize` at deliverable/engine/canvas.ts:71-75.

### One thing breaks on any vertical flow

Edge attachment is horizontal-only by default. `sidePoint` at deliverable/engine/render.ts:40-53 has this default branch:
...
It always returns the vertical centre and always picks a left or right face. Top and bottom are reachable only when a canvas author set the side explicitly (deliverable/engine/render.ts:46-47).

Generated canvases set no sides (deliverable/engine/iterations.ts:580). So every generated edge hits that default. On a top-to-bottom flow the arrows would leave and enter box sides instead of bottoms and tops. The fix is to compare vertical distance against horizontal distance and return the top or bottom face when vertical wins. It is a few lines, and it is required, not polish.

### The three options

**Option A — rotate the existing generated layout.** Make the milestone a horizontal band and stack milestones downward. States inside one milestone sit side by side. Change deliverable/engine/iterations.ts:576 to `x: row * W, y: col * H`, plus the vertical branch in `sidePoint`.
...
**Option B — lift version 2's ranker.** Take `stateRows` from deliverable/engine/project.ts:65-80 (reference `v2`), use it as a sort key and multiply the rank into a y-coordinate.

- Cost: roughly twenty lines, plus two corrections version 2 never made. It must actually sort, which version 2 never did. Its cycle break at deliverable/engine/project.ts:71 uses declaration order, so a forward edge that runs backward in the file is silently dropped from the layout. Replace that with a real topological order.
...
An architecture decision record in version 2 governs this and was never carried forward. It is at spec/ledger/se/adr-figures-derived-set.md, reference `v2`, adjudicated by a person.
...
Version 3 has no `spec/ledger/` directory at all. I listed `spec` and it holds exactly two entries: `expeditions` and `v3-plan.md`.
...
One live offline hazard sits in deliverable/engine/bin/mermaid-check.ts:37, a direct import from a content delivery network. It is a build-time syntax harness that writes an HTML file for a real browser, not panel code. It would break if copied into a panel.
...
One step is one file under deliverable/machines/rigor_matrix/rows/. The frontmatter carries everything:
...
A body `## Evidence form` section is refused outright (deliverable/engine/rigor-matrix.ts:68-74). The frontmatter is the single truth. That is an owner ruling from 2026-07-30 and it means the row grammar and the form-template grammar must stay separate.

### The columns

Five columns, four of them pinnable. From deliverable/engine/rigor-matrix.ts:17-18:
...
`compileColumn` at deliverable/engine/rigor-matrix.ts:214-289 tailors by deletion and contraction. Rows whose cell is `none` vanish. Every survivor's dependencies are resolved transitively through the struck ones, so the graph stays connected.
...
Monotonicity is tested: patch is a subset of minor is a subset of major (deliverable/tests/rigor-matrix.test.ts:121-130).

### What the compiler ignores

Only `none` has mechanical meaning. The check at deliverable/engine/rigor-matrix.ts:217 filters it, and the cell value is never read again. `full`, `tailored` and `inherit` are indistinguishable to the compiler. They reach the machine only as prose, spliced into guidance at deliverable/engine/rigor-matrix.ts:274.
...
**The killer flag is cosmetic.** It reaches the author inside an HTML comment (deliverable/engine/session.ts:301), and the checker strips HTML comments before looking (deliverable/engine/session.ts:327). A killer field is exactly a required field, and required already defaults to true.

**The escalation reopen ledger is inert.** The demands ledger and the reopen diff are computed correctly at deliverable/engine/iterations.ts:352-368. I read them. The comment above states the law: a filled step survives escalation only while its demand stands. Then deliverable/engine/session.ts:587 calls `pinIteration(this.root, it, size);` as a bare statement and discards the return value. The reopen list is written into the pin and read by nobody. The machinery to do it properly exists at deliverable/engine/machine.ts:172-251 and has exactly one caller, the unrelated jump-back path.

**One seed has no runner.** The step at deliverable/machines/rigor_matrix/rows/M8_10_fill-story-evidence.md declares `seeds: demos` and demands evidence that demos were seeded. No row anywhere declares `runs: demos`. Nothing will ever walk it.

**No iteration has ever been run.** Zero branches match `it/*`. There is no `spec/iterations/` directory — I listed `spec` and it is not there. Every claim about a pinned walk at runtime rests on unit tests against temporary directories. The `iter/*` branches on this machine belong to version 2 and are not evidence to the contrary.

### The first-iteration tension

The kickoff step instructs, in capitals, at deliverable/machines/rigor_matrix/rows/M0_90_gate-kickoff.md:70: "THE FIRST ITERATION OF A PRODUCT IS `product`: it authors the vision packet, the stakeholders and the actual state, and every later iteration inherits those by pointer."

`product` is the top of the size order. De-escalation is refused at deliverable/engine/iterations.ts:337-345, with the message "a prediction that proved too big is finished at its size".
...
The tick packet does not carry them. I read `tickInfo` at deliverable/engine/session.ts:2158-2190. It builds the agent's per-state object from identifier, kind, statement, guidance, priority, legal tools, entry, exit, exit status, pulled documents, lookahead reading and next edges. There is no evidence form key.
...
Counted mechanically across deliverable/machines/rigor_matrix/rows/:
...
`assertGateReport` at deliverable/engine/session.ts:281-350 is the only code that reads a step's field list. It has one call site, deliverable/engine/session.ts:2297, guarded by the state being a gate.
...
- the guard also tests that the machine identifier ends in `-walk`. That suffix holds only because deliverable/engine/iterations.ts:515 renames the machine compiled at deliverable/engine/rigor-matrix.ts:286. Rename the sub-machine and every gate report silently stops being checked.
- the kickoff state is built with kind `work` at deliverable/engine/iterations.ts:490, while being handed a gate row's five fields at :494. So `change_size` — the field that decides the whole machine's shape — is never checked.

### What "filled" means today

`const filled = content !== "";` — deliverable/engine/forms.ts:96, after HTML comments are stripped.
...
Version 1 solved this better, and not by banning "TBD". It made it a counted state. `tbdValue` at engine-go/schemas.go:202-204 recognises the marker; a field that is missing, empty or "TBD" counts as open; a node is undecided while any core field is open. Porting that predicate into deliverable/engine/forms.ts:96 closes the biggest hole with no type system at all.
...
5. **verdict** — one of a declared closed set, plus a mandatory reason. 4 fields. The gate verdict already exists as a string-prefix test at deliverable/engine/session.ts:335-336, which today accepts "PASSABLE".
6. **files** — paths that must exist in the record's evidence directory. About 7 fields. The check already exists at deliverable/engine/forms.ts:106-111, but at form level, not per field.
...
Version 1's pair is better than a plain boolean. It used `tier: core | deferrable` (engine-go/schemas.go, with the authored schemas under quackitect/method/config/schemas/). That distinguishes "blocks" from "counts but never blocks", which is what the ten "or an explicit none" fields are reaching for in prose.
...
- types and attributes — engine-go/schemas.go:24-32, covering enum, bool, string, int and pattern, with min, max, tier and default;
- a schema tester that refuses an unknown key, an unknown type, a malformed enum or a default outside its own enum — engine-go/schemas.go:390-391;
- the "TBD" predicate and the open-field rule — engine-go/schemas.go:202-232;
- per-field provenance and a traffic light, refusing self-reported confidence — engine-go/schemas.go:236-297;
- the generator — `applySchemaPrefill` at engine-go/mint.go:230-306.
...
- a confirm button per unconfirmed prefill, deliverable/engine/render.ts:1399;
...
- for a gate report only, a fully rendered scaffold handed back as write arguments, deliverable/engine/session.ts:304-310;
...
What the person needs and the agent must never have: the confirm button. The prefill law works only because an agent cannot press it. `prefills` at deliverable/engine/forms.ts:56-58 returns the comment blocks as a list, which is what makes confirmation one field at a time. That is the genuine invention here. No outside prior art does per-field human confirmation of machine-suggested content. The nearest protocol pattern gives a person accept or decline over a whole form.

### The migration cost, corrected

The research disagreed about this and the smaller number is right.

The pin hashes the evidence specification: `evidence: JSON.stringify(row.evidence_form)` at deliverable/engine/iterations.ts:356, compared at :362-368. Adding a `type` key to every field changes every hash.

The first pass concluded that every pinned iteration would reopen every filled step on the next tick. It would not. The comparison runs only inside the pin function, which refuses any non-escalating size first (deliverable/engine/iterations.ts:335-345). I read that guard.
...
2. **The version 2 figures decision.** Does spec/ledger/se/adr-figures-derived-set.md still bind version 3? Yes or no. If yes, it should be restated somewhere version 3 can see it.
3. **Vertical edge attachment.** Add the top and bottom faces to the default in deliverable/engine/render.ts:48-51? Yes or no. Any vertical layout needs it.
