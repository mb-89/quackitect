...
| --- | --- | --- | --- | --- |
| the se lane is reachable | an MCP server entry | `.mcp.json` | `--additional-mcp-config @.copilot/mcp-config.json` | `.vscode/mcp.json` |
...
SO THIS IS A BUILD. The agent-agnostic source stays in `guidance/`, and
...
agent-agnostic source of truth stays in `guidance/`, and an installer
