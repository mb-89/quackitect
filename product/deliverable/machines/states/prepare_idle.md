...
guidance: Preflight and the SMOKE test — the exit scripts run them, and both are quick. Tick to run; it takes about a second and the bar shows how far it has got. A failure is named in the refusal. While a check stands red, the repair tools are legal HERE — fix what the output names, then tick again.
---
