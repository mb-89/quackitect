...
test("a detour that never came home is left hanging, not merged", () => {
  for (const status of ["obsolete", "reverted", "deferred"] as const) {
    const g = decisionsAsGitGraph([
      node({ id: "d1", at: "2026-07-31T10:00:00Z" }),
      node({ id: "d2", at: "2026-07-31T10:01:00Z", parent: "d1", status, closed_at: "2026-07-31T10:02:00Z" }),
    ]);
    ...
    assert.match(g, new RegExp(`tag: "${status}"`), "and the commit says what became of it");
  }
});
...
test("the markdown page fences the graph so the preview renders it", () => {
  ...
  assert.match(md, /```mermaid\ngitGraph/, "the fence is what VS Code renders natively");
