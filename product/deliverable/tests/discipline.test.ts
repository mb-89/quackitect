...
test("the classifier names the lane tool for each harvested shell shape", () => {
  ...
  assert.equal(classifyCommand("Get-Content product/engine/toll.ts")?.tool, "se_file_read");
...
function productRoot(): string {
  ...
  mkdirSync(join(root, "product", "deliverable", "tests"), { recursive: true });
  for (const m of ["pull", "files", "search"]) {
    writeFileSync(join(root, "product", "deliverable", "engine", `${m}.ts`), `// ${m}\n`);
    writeFileSync(join(root, "product", "deliverable", "tests", `${m}.test.ts`), `// ${m} test\n`);
  }
...
test("the battery refuses while every change maps to a test file, and names the scope", () => {
  ...
  writeFileSync(join(root, "product", "deliverable", "engine", "pull.ts"), "// changed\n");
  try {
    batteryGate(se, root, false);
    assert.fail("expected the scope refusal");
  } catch (e) {
    assert.ok(e instanceof Rejection && e.clause === "SE-C-131");
    assert.deepEqual((e as Rejection).remedy.args, { files: ["product/deliverable/tests/pull.test.ts"] }, "the remedy IS the scoped call");
  }
  rmSync(root, { recursive: true, force: true });
});
...
test("an unmapped change buys the battery — no scoped run answers for it", () => {
  ...
  const se = join(root, ".se");
  testRecord(se, root, true);
  writeFileSync(join(root, "product", "deliverable", "engine", "render.ts"), "// no test file exists for render\n");
...
test("piecemeal past the threshold flips: scoped refuses toward the battery, the battery is granted", () => {
  ...
  for (let i = 0; i < threshold - 1; i++) {
    testRecord(se, root, true, `scope-${i}`, [`product/deliverable/tests/f${i}.test.ts`]);
  }
  assert.doesNotThrow(() => scopedGate(se, root, ["product/deliverable/tests/f0.test.ts"], false), "already-seen files do not advance the odometer");
  try {
    scopedGate(se, root, ["product/deliverable/tests/fresh.test.ts"], false);
  ...
  testRecord(se, root, true, "scope-x", [`product/deliverable/tests/fresh.test.ts`]);
  assert.doesNotThrow(() => batteryGate(se, root, false));
  // And a battery run resets the odometer.
  testRecord(se, root, true);
  assert.doesNotThrow(() => scopedGate(se, root, ["product/deliverable/tests/f0.test.ts"], false));
  rmSync(root, { recursive: true, force: true });
});
...
test("the unchanged gate is PER SCOPE — pull's green does not fence files' run", () => {
  const root = productRoot();
  const se = join(root, ".se");
  testRecord(se, root, true, "pull-scope", ["product/deliverable/tests/pull.test.ts"]);
...
test("the change-to-test map is by name and refuses to guess", () => {
  ...
  const r = mapChangedToTests(root, [
    "product/deliverable/engine/pull.ts",
    "product/deliverable/tests/files.test.ts",
    "product/guidance/voice.md",
  ]);
  assert.deepEqual(r.mapped, ["product/deliverable/tests/files.test.ts", "product/deliverable/tests/pull.test.ts"]);
  assert.deepEqual(r.unmapped, ["product/guidance/voice.md"]);
