...
export function appendNote(seDirPath: string, text: string): { captured: string; inbox: number } {
  const p = notesPath(seDirPath);
  const note: StrayNote = { ref: `note-${randomBytes(6).toString("hex")}`, text, at: new Date().toISOString() };
