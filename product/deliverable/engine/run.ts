...
export function startRun(root: string, log: CallLog, command: string, cwd: string): RunHandle {
  ...
  const script = [
    ...
    `const r=spawnSync(process.argv[1],{shell:true,cwd:process.argv[2],encoding:"utf8",timeout:600000,windowsHide:true});`,
  ...
  const child = spawn(process.execPath, ["-e", script, command, cwd, doneFile], { detached: true, stdio: "ignore", env, windowsHide: true });
  child.unref();
...
export function runCommand(log: CallLog, command: string, cwd: string): CallRecord {
  ...
  const env = { ...process.env };
  delete env.SE_SESSION_FILE;
  const r = spawnSync(command, { shell: true, cwd, encoding: "utf8", timeout: 10 * 60 * 1000, env, windowsHide: true });
