...
function order(a: unknown, b: unknown, op: string): number {
  ...
  throw new Rejection({
    ...
    got: `${ta} ${op} ${tb}`,
    remedy: { tool: "se_file_read", args: { path: "product/spec/bases-syntax.md" }, note: "section 7 says ordering works on numbers and dates" },
    source: SRC,
  });
}
...
export function parseDuration(text: string): Duration {
  ...
  if (parts === null) {
    throw new Rejection({
      ...
      remedy: { tool: "se_file_read", args: { path: "product/spec/bases-syntax.md" }, note: "the unit table is in section 7" },
      source: SRC,
    });
  }
  let months = 0;
  ...
  if (m === null) {
    ...
    if (Number.isNaN(loose.getTime())) {
      throw new Rejection({
        ...
        got: JSON.stringify(text),
        remedy: { tool: "se_file_read", args: { path: "product/spec/bases-syntax.md" }, note: "date() parses YYYY-MM-DD HH:mm:ss" },
        source: SRC,
      });
    }
    return loose;
...
function lex(src: string): Tok[] {
  ...
  while (i < src.length) {
    ...
    if (c === '"' || c === "'") {
      ...
      if (j >= src.length) {
        throw new Rejection({
          ...
          got: src,
          remedy: { tool: "se_file_read", args: { path: "product/spec/bases-syntax.md" }, note: "strings take single or double quotes" },
          source: SRC,
        });
      }
    ...
    throw new Rejection({
      ...
      remedy: { tool: "se_file_read", args: { path: "product/spec/bases-syntax.md" }, note: "section 7 lists every operator" },
      source: SRC,
    });
  }
  out.push({ k: "end" });
...
class Parser {
      ...
      if (this.eat(".")) {
        ...
        if (t.k !== "id") {
          throw new Rejection({
            ...
            remedy: { tool: "se_file_read", args: { path: "product/spec/bases-syntax.md" }, note: "section 4 covers property references" },
            source: SRC,
          });
        }
        this.pos++;
  ...
  private primary(): Node {
      ...
      for (;;) {
        ...
        if (key.k !== "str" && key.k !== "id") {
          throw new Rejection({
            ...
            got: JSON.stringify(this.src),
            remedy: { tool: "se_file_read", args: { path: "product/spec/bases-syntax.md" }, note: "section 8 covers objects" },
            source: SRC,
          });
        }
        this.pos++;
    ...
    throw new Rejection({
      ...
      remedy: { tool: "se_file_read", args: { path: "product/spec/bases-syntax.md" }, note: "section 8 lists every type literal" },
      source: SRC,
    });
  }
}
...
function formulaValue(name: string, ctx: Ctx): unknown {
  ...
  if (src === undefined) {
    throw new Rejection({
      ...
      got: `formula.${name}`,
      remedy: { tool: "se_file_read", args: { path: "product/spec/bases-syntax.md" }, note: "section 3 covers formulas" },
      source: SRC,
    });
  }
...
function multiply(a: unknown, b: unknown): unknown {
  ...
  if (b instanceof Duration) {
    throw new Rejection({
      ...
      remedy: { tool: "se_file_read", args: { path: "product/spec/bases-syntax.md" }, note: "section 7 states the ordering rule" },
      source: SRC,
    });
  }
  return toNumber(a) * toNumber(b);
...
function call(node: Node & { k: "call" }, ctx: Ctx): unknown {
  if (node.recv === null) {
    ...
    if (fn === undefined) {
      throw new Rejection({
        ...
        got: `${node.name}()`,
        remedy: { tool: "se_file_read", args: { path: "product/spec/bases-syntax.md" }, note: "section 9 lists every function; register our own with registerGlobal" },
        source: SRC,
      });
    }
  ...
  if (fn === undefined) {
    ...
    throw new Rejection({
      ...
      got: `${t}.${node.name}()`,
      remedy: { tool: "se_file_read", args: { path: "product/spec/bases-syntax.md" }, note: "section 9 lists every function; register our own with registerMethod" },
      source: SRC,
    });
  }
  guardVolatile(fn, node.name, ctx);
...
function guardVolatile(fn: Fn, name: string, ctx: Ctx): void {
  ...
  throw new Rejection({
    ...
    remedy: { tool: "se_file_read", args: { path: "product/spec/bases-syntax.md" }, note: "pass deterministic: false where a moving value is wanted" },
    source: SRC,
  });
}
