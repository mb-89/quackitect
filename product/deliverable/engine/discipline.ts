...
export function suiteFiles(root: string): string[] {
  const dir = join(root, "product", "deliverable", "tests");
  try {
    return readdirSync(dir)
      .filter((f) => f.endsWith(".test.ts"))
      .map((f) => `product/deliverable/tests/${f}`)
      .sort();
  } catch {
    return [];
  }
...
export function mapChangedToTests(root: string, changed: string[]): { mapped: string[]; unmapped: string[] } {
  ...
  for (const rel of changed) {
    ...
    const m = p.match(/^product\/deliverable\/engine\/([\w-]+)\.ts$/);
    const candidate = m !== null ? `product/deliverable/tests/${m[1]}.test.ts` : undefined;
