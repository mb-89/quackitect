...
---
<!-- design: method-criteria-items  implements: req-criteria-validation, req-criteria-in-needs :: Success criteria live ON the need (owner ruling 2026-07-08): each need carries its own `## Success criteria` pass lines, the standalone crit- node type is retired, the criteria view renders every need with that section, and the ch5 validation view traces the same rows to their outcomes - the V-model's outer arc as data. -->
## The big idea
<!-- fill [mandatory]
