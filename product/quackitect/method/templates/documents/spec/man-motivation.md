...
Contents: two authored moves - the gap as a claim (what every existing
  ...
  the needs register. Where an alternative COMPOSES with this system rather
  than competing, say so and name the seam. The success criteria live ON the
  needs: each need carries its pass lines, one expand away in the design
  input register.
...
Form: prose for the two moves; the needs render as the register's top rows.
...
<!-- ai:3 -->
The idea above composes into a set of needs the system has to serve. They are the top rows of the design input register: each names the stakeholder it serves, and its expand carries the pass lines that accept it. A row without a stakeholder is a wish, not a need.
---
## Business case
<!-- fill [judgment]
