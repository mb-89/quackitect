...
<!-- design: method-runme-orientation  implements: req-runme-orientation.1, req-runme-orientation.2, req-runme-orientation.3 :: RUNME.ps1 and RUNME.sh follow one contract: check, install, verify, orient - never build a project. Check looks for each dependency before touching anything. Install runs Winget on Windows, or the package manager with the go.dev tarball as a fallback on Linux. Verify runs `quack version`, which bootstraps the global binary from this repo's vendored source. Orient prints the next steps once verification succeeds - starting a project through the agent, or through `quack start stubs`, and where the book's onboarding chapter lives. Neither script creates a workspace or a project: a deterministic installer carries no LLM and cannot meaningfully author one. Windows only opens the book with the explicit `-OpenBook` flag; without it, the script just prints the path. Linux stays headless and only prints paths, so a CI log shows them and the exit code stays honest. -->
