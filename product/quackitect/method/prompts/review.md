...
6. **Spot determinizer candidates.** Read the logs — they live in the workspace DATA HOME, never the repo: `<data-home>/logs/` (`quack version` prints the exact path; a `logs_dir` key in the GLOBAL user config overrides it). Which scripts did the AI write that the determinizer could own? A script that failed a few times is a strong lead. The fix may be a new tool. It may be better guidance.
