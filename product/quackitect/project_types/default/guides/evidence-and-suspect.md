...
- **Grounded-review** — the claim cannot run. It points at an inspectable referent (file:line). A user reads it.
- **Judgment** — the claim is about taste or fit. A user decides.

On any input change a check goes SUSPECT, not open. A user blesses it back to DONE. The bless re-attests it against the new inputs.
