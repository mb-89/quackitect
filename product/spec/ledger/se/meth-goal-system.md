---
id: se.meth-goal-system
kind: method
statement: "The goal system: goals, their conflicts, their priorities - conflicts named openly, priority order ruled."
provenance:
  iteration: i2g-tutorial-machine
  ai_involvement: agent-drafted
---

## Situation
Inside the vision draft (M1). A vision without a goal system hides its trade-offs; hiding them poisons every later decision.

## Procedure
- List the goals the to-be world drives toward. Three is a good number.
- Order them: which wins when they collide.
- Name each conflict openly as a pair (trust against speed; durability against freedom) with the accepted price.
- The conflicts feed the risk log; the priority order feeds every later criteria weighting.

## Sources
Goal systems per methodische Entwicklung (Lindemann); the v1 motivation chapter carries a worked example.
