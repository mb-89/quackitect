---
id: se.machine-systematic-write-requirements
kind: machine_state
statement: "Write the requirements in full: EARS, four kinds, detail included - verify_method named on every one."
provenance:
  iteration: i2g-tutorial-machine
  ai_involvement: agent-drafted
machine: se.machine-systematic
state: write_requirements
state_kind: work
filled_by: agent
---

## Guidance
Derive from the use-case steps and extensions. EARS shapes ([[meth-ears]]); kinds functional / quality / constraint / interface - qualities carry six-part scenarios ([[meth-quality-scenarios]]), constraints link their binding norm. Naming the verify_method (test / analysis / inspection / demonstration) IS the verifiability check - unnameable means rewrite or drop. breaks_if_removed mandatory on every requirement. source_refs to stakeholders, stories, norms. Detail now - no TBD survives this milestone; requirements are design input, never build-time afterthoughts. Position the concrete set against the standard checklists for this deliverable kind ([[meth-state-of-the-art]]). Expect iteration with the functions ([[meth-twin-peaks]]).

## Evidence form
- register | the requirement set: EARS, kinds, verify_method and breaks_if_removed on every row | required
- set_criteria | complete, consistent, affordable, bounded - argued | required
