...
## Situation
Every gate state links this note. The gate's own evidence form carries its milestone-specific acceptance items; this template carries the standard review that evaluates them. The compiler will inject the standard fields into every gate's form (single source); until then, fill them from here.
