---
id: se.meth-frame-tactics
kind: method
statement: "FRAME - Feasibility, References, Alternatives, Modeling, Elements/structure: the architecting tactic toolbox spanning M4 and M5, not a sequence."
provenance:
  iteration: i2g-tutorial-machine
  ai_involvement: agent-drafted
---

## Situation
No single architecture method is general enough; FRAME names the five tactic groups and where each bites in the machine.

## The groups
- F Feasibility: early convergence checks, economically reasonable - [[meth-feasibility-checks]]. Feeds candidate composition.
- R References: proven knowledge - heuristics, patterns, catalogs, standards, benchmarking, reference architectures, TRIZ. Feeds enumeration.
- A Alternatives: always compare - the 8-step decision model ([[meth-eight-step-decision]]) spanning criteria to selection; morphological analysis + design-space exploration at EVERY design level.
- M Modeling: express candidates so they communicate - matrices first, derived views as needed.
- E Elements & structure: optimize the cut - DSM operations, modular vs integral, layering; increase cohesion, decrease inter-element dependencies; make/reuse/buy per element with early supplier involvement.

## Sources
Kreuter & Palm, architecting-tactics program (2021), re-expressed.
