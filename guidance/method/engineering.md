...
- THE CHECKER is `tsc -p . --noEmit` from `deliverable` (the
...
- THE LINTER-FORMATTER is Biome, one binary for both. Config:
  `deliverable/biome.json`. House choices there:
    - lineWidth 140.
...
`deliverable/tests/help.test.ts`.
...
- WHAT SHOWS UP IN VS CODE: engine-served surfaces (the bar, the machine
  ...
  extension edit: `npm run build` in deliverable, robocopy
