...
func itemsOfTickets(rows []ticketRow) []tree.Item {
	items := make(map[string]*tree.Item, len(rows))
	...
	out := make([]tree.Item, 0, len(roots))
...
// [[spec/design_output/tui#the-work-tab]]
func itemOfTicket(one ticketRow) tree.Item {
	kind := KindTicket
	if one.Route == KindGroup {
		kind = KindGroup
	}
	return tree.Item{Name: one.Name, Keys: map[string]string{
		"kind":     kind,
		"path":     one.Path,
		"state":    one.State,
		"step":     one.Step,
