...
// The cursor set on one column, so a caller opens an edit where it says. [[spec/design_output/tree-view#a-cell-takes-an-edit]]
func (t *Tree) CursorTo(col int) {
	t.cur = max(0, min(col, len(t.Cols)-1))
}
