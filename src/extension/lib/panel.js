...
function tree(model) {
  return [
    ...
    `<button class="fold" data-fold="open" title="open every group">${markOf("U+229E")}</button>`,
    `<button class="fold" data-fold="shut" title="shut every group">${markOf("U+229F")}</button>`,
    "</div>",
    '<div class="tree">',
...
function style(groups) {
  return [
    ...
    ".find { display: flex; align-items: center; gap: 4px; margin: 2px 0 6px 0; }",
    ".fold { background: var(--vscode-sideBar-background, transparent); border: 1px solid var(--vscode-contrastBorder, transparent);",
    "  color: var(--vscode-foreground); cursor: pointer; padding: 0 4px; }",
    ".filter { flex: 1; min-width: 0; box-sizing: border-box; margin: 0;",
