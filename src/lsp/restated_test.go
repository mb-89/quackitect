...
// A heading of its own says what the chapter under the pointer leaves unsaid. [[spec/design_output/lsp#a-second-copy-draws]]
...
// One rule in two guidance notes drifts, so the rule draws on the second. [[spec/design_output/lsp#a-second-copy-draws]]
...
// Two rules of two notes say two things, so the rule stands quiet. [[spec/design_output/lsp#a-second-copy-draws]]
...
// A tree carrying no bound draws nothing, the way a name bound of nothing reads. [[spec/design_output/lsp#a-second-copy-draws]]
func TestABoundOfNothingHoldsItsRuleOff(t *testing.T) {
	tree := fixture(t, map[string]string{
		"spec/design_output/one.md": "# The stop hook holds a turn\n\nA line stands here.\n",
		"spec/guidance/two.md": "# The stop hook holds a turn\n\n" +
			"For details, see [[spec/design_output/one#the-stop-hook-holds-a-turn]].\n",
		"spec/guidance/three.md": "# Actionables\n\n" +
			"1. Reach the outside through a door under src/doors, and nowhere else.\n",
		"spec/guidance/four.md": "# Actionables\n\n" +
			"1. Reach the outside through a door under src/doors, and never otherwise.\n",
	})

	if found := rules(restatedFaults(tree, 0, 0)); len(found) != 0 {
		t.Fatalf("a bound of nothing draws nothing, and this answers %v", found)
	}
	if found := rules(restatedFaults(tree, 4, 0)); len(found) != 1 || found[0] != RestatedPointer {
		t.Fatalf("one bound alone draws its own rule, and this answers %v", found)
	}
}

// Every front asks for one path, so the tree pays the whole pass once. [[spec/design_output/lsp#a-second-copy-draws]]
func TestTheTreePaysTheRestatedPassOnce(t *testing.T) {
	tree := fixture(t, map[string]string{"spec/guidance/one.md": "# Actionables\n\n1. Reach a door.\n"})
	ran := 0
	pass := func() []Finding {
		ran++
		return []Finding{fault(RestatedRule, "spec/guidance/one.md", 1, "a finding")}
	}

	for at := 0; at < 3; at++ {
		if found := tree.Restated(pass); len(found) != 1 {
			t.Fatalf("the pass answers its findings every time, and this answers %v", found)
		}
	}
	if ran != 1 {
		t.Fatalf("three asks cost one pass, and they cost %d", ran)
	}

	tree.Holds("spec/guidance/one.md", "# Actionables\n\n1. Reach another door.\n")
	if tree.Restated(pass); ran != 2 {
		t.Fatalf("a changed buffer pays the pass again, and it costs %d", ran)
	}
}

// The measure answers the run, and a code span or a link counts for nothing. [[spec/design_output/lsp#a-second-copy-draws]]
