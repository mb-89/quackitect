...
const rules = readdirSync(JUDGED)
  ...
  .map((n) => ({ ...readRule(readFileSync(join(JUDGED, n), "utf8")), name: n.replace(/\.yml$/, "") }));
...
test("a rule file reads into scalars and a list", () => {
  ...
  assert.deepEqual(read, {
    message: "say a thing", level: "error",
    labels: ["one", "two"], refuses: "two", max: 3, on: true,
  });
});
...
test("every judged rule carries what the judge needs", () => {
  ...
  for (const rule of rules) {
    ...
    assert.ok(Array.isArray(rule.labels) && rule.labels.length >= 2, `${rule.name} offers labels`);
    assert.ok(rule.labels.includes(rule.refuses), `${rule.name} refuses one of its own labels`);
...
test("the judge reads every warmup write, then samples, and a breach resets it", () => {
  ...
  assert.ok([judge.reads(), judge.reads(), judge.reads()].includes(false),
