...
// A SMALL YAML READER, FOR THE OWNER'S FORMAT.
//
...
func readable(text string) ([]yline, error) {
	var out []yline
	for i, raw := range strings.Split(strings.ReplaceAll(text, "\r\n", "\n"), "\n") {
		...
		out = append(out, yline{no: i + 1, indent: len(raw) - len(trimmed), text: trimmed})
	}
	return out, nil
}
...
func (p *yparser) sequence(indent int) (any, error) {
	...
	for p.at < len(p.lines) {
		...
		if k, _, isPair := strings.Cut(item, ":"); isPair && !looksQuoted(item) && k != "" {
			p.lines[p.at] = yline{no: l.no, indent: indent + 2, text: item}
...
// Reaching into a parsed tree, without a type switch at every call site.
