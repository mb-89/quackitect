...
func runAsk(c *call) int {
	...
	fs.SetOutput(c.err)
	fs.Usage = func() {
		fmt.Fprintln(c.err, "se ask - ask the index a question. Prints rows as JSON.")
		fmt.Fprintln(c.err, "")
		fmt.Fprintln(c.err, "  se ask --sql \"SELECT id, json_extract(front, '$.status') FROM note WHERE kind = 'work-token'\"")
		fmt.Fprintln(c.err, "  se ask --search \"heredoc\"        notes whose body holds the words")
		fmt.Fprintln(c.err, "  se ask --links wk-1234567890     what links to it, what it links to")
		fmt.Fprintln(c.err, "  se ask --dangling                links that reach nothing")
		fmt.Fprintln(c.err, "  se ask --schema                  the tables, as they are made")
		fmt.Fprintln(c.err, "")
		fmt.Fprintln(c.err, "  The index is read-only here. fresh says whether the engine is")
		fmt.Fprintln(c.err, "  watching the tree; when it is not, the rows may be behind the files.")
		fmt.Fprintln(c.err, "")
		fs.PrintDefaults()
	}
