...
func answerModel(conn net.Conn, m *model) {
	...
	for in.Scan() {
		...
		// A VERB MAY TAKE AS LONG AS A VERB TAKES: the run verb runs a test
		// suite. Its deadline is the verb's, and the client waits as long.
		if ask.Method == "verb" {
			_ = conn.SetDeadline(time.Now().Add(verbBudget)) // a deadline it cannot set is a wait the verb's own ceiling ends
		}
