...
// A CLONE BEHIND THE BRANCH READS ITS OWN spec/work, AND THERE A CLOSED TOKEN
// STILL READS OPEN.
//
...
// note carried both checklists answered, and spec/work/archive.jsonl named the
...
// bringing spec/work into step is the person's, and the pull only declines to
...
const archiveOnBranch = "spec/work/archive.jsonl"
...
// open note for one token at once, so bringing spec/work into step changes
...
//
// A REMEDY NAMES A FILE AND NOT A FOLDER. The stale copy is under spec/work on
// one box and under .se/work on another, and only the first is a lag a fetch
// closes. Where the token has no file at all, spec/work is the folder it would
...
// moves what is stale there. Sending that reader to spec/work sends them to a
// folder with nothing in it.
//
// MEASURED, September 2026. Three ids were named on every pull of a session
// under the fetch remedy, and spec/work held none of the three. All three sat
...
func behindNotice(r Roots, branch string, behind []string) string {
	...
	if len(lagging) > 0 {
		said += fmt.Sprintf("\n\nPassed over, because %s has archived them and this clone is behind it:\n  %s\n\n"+
			"They are not open work. Bring spec/work into step with %s, and pull again.",
