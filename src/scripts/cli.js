...
import { actionables, onlyOf, standingLayer } from "../level0/lib/guidance.js";
...
function standing() {
  ...
  const cloud = Boolean(process.env.CLAUDE_CODE_REMOTE || process.env.SE_CLOUD);
  const notes = readdirSync(GUIDANCE)
    .filter((n) => n.endsWith(".md"))
    .map((n) => ({ name: n, text: readFileSync(join(GUIDANCE, n), "utf8") }))
    .filter(({ text }) => {
      const only = onlyOf(text);
      return (only !== "cloud" || cloud) && (only !== "desk" || !cloud);
    });
