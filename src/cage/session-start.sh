...
# box is comes off one table, src/cage/hosts.json, through the one door that
# reads it.
#
...
# desk cannot mistake src/cage/cloud-runner.md for something addressed to it.
if ! node src/cage/host.mjs --cloud; then
  say "this is $(node src/cage/host.mjs --say). The extension starts the engine here, and src/cage/cloud-runner.md is a card for a cloud box and is not addressed to you."
  exit 0
fi
...
say "this is $(node src/cage/host.mjs --say), on commit $head of $branch"
