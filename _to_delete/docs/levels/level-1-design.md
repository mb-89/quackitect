...
**An agent is idle when no token is assigned to it.** That is the whole definition, and it
is answerable from the ledger at any moment.

| Entry | Ruling | Why | Breaks without it |
|---|---|---|---|
| **Idleness is read from the tokens, never from the harness** | **ACCEPTED** | The tokens already hold the answer. A second source can disagree with the first, and then something has to decide which is right. | Two answers to one question, and the harness one is the one that cannot be checked. |
| The harness idle event may be a **wake-up**, and nothing more | **ACCEPTED** | Same shape as a file-change event. It says look now. It does not say what is true. Losing one costs a moment. | An optimisation quietly becomes the mechanism, and the mechanism only works on one harness. |
| Nothing may **depend on it firing** | **ACCEPTED** | It exists on one harness. The design is built on what both provide, so this can only ever be an accelerator. | The crew works on one host and stalls on the other, for a reason nobody can see. |
| A crew is **despawned by its scope closing**, not by going idle | **ACCEPTED** | An idle walker with an open scope is waiting, not finished. Waiting is a normal state and it must survive. | An agent that pauses gets torn down, and its scope is left with nobody holding it. |
