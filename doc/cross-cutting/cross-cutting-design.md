...
## 4i. Never hand text to a shell

**A program is run by naming it and listing its arguments.** Building a command line for a shell puts a parser
between the intention and the act. That parser differs from shell to shell.

| Entry | Ruling | Why | Breaks without it |
|---|---|---|---|
| **Program plus argument list, never a command line** | **ACCEPTED** | The list arrives as written. Nothing splits it, nothing strips a quote, and nothing has an opinion about a character in a path. | Every path with a space, a quote or a drive letter becomes a defect, and the defect is in a different place on each platform. |
| The shells **disagree, and that is not fixable by quoting harder** | **ACCEPTED** | A PowerShell line that begins with a quoted path reads as a string, not a command. It needs a call operator that no other shell wants. Quoting correctly for all of them at once is not a thing that exists. | A quoting rule that works until somebody uses a different shell. |
| Where a line of text is **unavoidable**, it is built for one named shell | **ACCEPTED** | Some interfaces take only a line. Then the shell is chosen, not assumed, and the escaping is written for that one. | Escaping written for whichever shell the author happened to be using. |
| A path is **never concatenated into a string** that something else will parse | **ACCEPTED** | This is the same rule one level down, and it is where the failure usually starts. | The quoting is correct and the path was already broken before it got there. |

---

## 4h1. One parameter tree

**Everything that changes how the system runs is one tree.** Not a settings file here and a
configuration file there. One declaration, one store, one place to look.

**The panel is a subtree of it.** A group marked as shown becomes a section in the sidebar.
Adding a section is marking a group, and nothing about the panel is written by hand.

The idea is pyqtgraph's parameter tree. It keeps the data apart from the view, so one
declaration can be shown, stored and read back.

| Entry | Ruling | Why | Breaks without it |
|---|---|---|---|
| **One tree holds the whole configuration** | **ACCEPTED** | A value belongs to the system, not to the surface that happens to show it. | Two stores, two formats, and a value that is in both and different. |
| **The panel is a subtree**, chosen by a flag on a group | **ACCEPTED** | The sidebar is one view over the tree. Making a new section is marking a group. | The panel and the configuration drift, and every new control is a code change. |
| **The word "setting" is not used** | **ACCEPTED** | v3 kept interface state in a file called settings, and it read as though the interface was configuring the editor. Both meanings then lived in one word. | A name that hides which of two things is meant, in the file a newcomer opens first. |
| A value is a **parameter**. A parameter shown in the panel is **shown**. That is the whole vocabulary | **ACCEPTED** | Two words, both plain, neither overloaded. | Terms multiply, and the same thing is called three names in three files. |
| **The editor's own settings file holds nothing of ours** | **ACCEPTED** | It is the editor's, for configuring the editor. Our values are ours. | The reader cannot tell which entries belong to which system. |
| **Actions are nodes too** | **ACCEPTED** | pyqtgraph has an action parameter, which renders as a button. A panel is not only values, and a declaration that covers half of one is worse than either. | Half the panel declared and half written. |
| The type words are **pyqtgraph's and JSON Schema's** | **ACCEPTED** | group, bool, int, float, str, list, action. Nothing about types is invented. | A private vocabulary that has to be learned and documented. |
| The **engine validates every change** | **ACCEPTED** | Type, limits, and narrowing are one rule set. The panel is a view, and a view that validates is a second rule set. | Two answers to whether a value is allowed. |
| The view **keeps no value of its own** | **ACCEPTED** | Every change goes to the engine and comes back. What is on screen is what is stored. | Two copies of one number, and no rule for which wins. |
| A declaration that **cannot be read leaves one button** | **ACCEPTED** | The one that opens the page explaining how to fix it. | A blank panel, and no route out of it. |
| **Two files, and each has one job** | **ACCEPTED** | The declaration is authored and read only. The values are written by the engine and hold only what differs from a default. One file would put generated content in a file a person edits. | Either the tool writes over what somebody wrote, or defaults are copied into every project. |
| **Both directions work, and both are watched** | **ACCEPTED** | Change a value in the panel and the file is written. Edit either file and the panel follows. Neither is the master, because the view holds nothing. | The panel and the file disagree, and the person has to know which one is real. |
| A change to the **declaration rebuilds the panel**. A change to the **values only fills it** | **ACCEPTED** | They are different kinds of change, and rebuilding on every value change would lose focus mid-edit. | Typing in a field re-renders the field. |
| **Which groups are shown is itself a parameter** | **ACCEPTED** | Choosing what the panel holds is setting a value, in the same tree and the same store. Nothing about the panel is kept anywhere else. | A second store, for the one thing the panel needed to remember. |
| The panel shows **one group by default** | **ACCEPTED** | A panel that opens with everything in it is a panel nobody reads. The rest are one click away. | The first thing a newcomer sees is a wall. |
| Sections are **collapsible** | **ACCEPTED** | The sidebar is narrow, and a group that is not being used costs its whole height. | Choosing a group means accepting its height for ever. |
| A heading is **written once** | **ACCEPTED** | The editor already draws the view's name. Repeating it inside is the same words twice, in a narrow column. | Two headings, and the second one is ours. |
| The view **says when it can listen** | **ACCEPTED** | A message sent before the view is ready is lost, and the panel comes up with empty fields. It is a race, and a handshake is the only fix that always works. | Values appear or do not, depending on how fast the machine is that day. |
| The **grid survives every build** | **ACCEPTED** | Five columns, columns that do not collapse, and the label in the first. It is the renderer's contract, not a habit. | Each new row invents a layout, which is what the declaration was meant to stop. |

---

## 4h. The status button

**One control is the thing you press and the place you look.** A separate status
row costs a line and puts the question and the answer in different places. It also goes
stale on its own. The light lives inside the button that acts on it.

| Entry | Ruling | Why | Breaks without it |
|---|---|---|---|
| **A control that can be in a state shows that state inside itself** | **ACCEPTED** | Press and read are the same act. Two places to look means one of them is eventually wrong. | A panel of labels that describe a panel of buttons, kept in agreement by hand. |
| **Four states, and no more**: idle, busy, faulty, running | **ACCEPTED** | These are the four a person acts on differently. A fifth is a detail, and details belong in the tooltip. | A colour vocabulary nobody can hold in their head. |
| Grey, yellow, red, green — **in that order of severity** | **ACCEPTED** | Nothing running, waiting, wrong, working. Reading them needs no legend. | Colour that has to be learned before it can be used. |
| **Running beats.** The beat is a real event, never a decoration | **ACCEPTED** | A light that pulses on a timer says the panel is alive. A light that pulses when the thing proves itself says the thing is alive. Only the second is worth the pixels. | An animation that keeps running while the thing behind it is dead. |
| **Waiting has a budget**, and missing it turns the light red | **ACCEPTED** | A light that stays yellow for ever tells a person that something is wrong, late. | The failure state is reached by giving up rather than by measuring. |
| **Pressing a red control tries again** | **ACCEPTED** | It is what a person reaches for. Making them find a separate restart is making them look for something the design already knows they want. | Recovery lives somewhere the user has to be told about. |
| The label says the **act, not the object** | **ACCEPTED** | "start", not "start the engine". The panel already says what this section is about, and a label that does not fit is cut. | Labels truncated by their own container, which is how it looked before this rule. |
| **The state is asked of the thing, never remembered** | **ACCEPTED** | For the engine, the record's own modification time is the proof it is writing. Cheap, and it cannot disagree with reality. | The panel keeps a belief about a process, and the belief outlives the process. |

---

## 4g. The rule of five

**At most five entries wherever a person looks.** A folder with twenty things in it is not
organised, it is a pile. The limit forces the question the pile avoids: what are the few
things this level is actually about.

| Entry | Ruling | Why | Breaks without it |
|---|---|---|---|
| **Five entries, at every level a person might open** | **ACCEPTED** | Five is small enough to read without scanning and large enough to be honest. It applies to folders and to files alike. | The top of the tree becomes an inventory, and a newcomer reads all of it to find one thing. |
| **Names beginning with a dot do not count** | **ACCEPTED** | They are machinery. A person does not choose to look at them. | The budget is spent on things nobody reads. |
| Output is **not source**, and it lives in a dotted folder | **ACCEPTED** | It is machinery by the rule above, it is rebuilt from source, and it does not belong in version control. | Built files sit beside the files they were built from, and a reader has to know which is which. |
| **Source is one entry**, whatever languages are inside it | **ACCEPTED** | A language is not a concern of the person opening the tree. | The top level grows one entry per language, which is a fact about us and not about the work. |
| **The tools are one entry** | **ACCEPTED** | Installing, linting and generating are all the same kind of thing: something you run, not something that runs. | Each tool claims its own place at the top. |
| Depth is **paid for**, so the surface names the entry point | **ACCEPTED** | Pushing things down is what keeps the count low, and it makes the first useful command harder to find. The document at the surface has to say it. | Setup is one level deeper than it was, and nobody finds it. |
| **Dated, situational material is private by default** | **ACCEPTED** | A field report is a measurement of one machine on one day. It is evidence for a decision, not part of the method, and it may carry what the machine happened to hold. | Situational material travels with the method, and something private leaves with it. |
| Private material lives in the **dotted private folder**, and never in version control | **ACCEPTED** | Already ruled for sources. A report is the same kind of thing. It also keeps the count down for free. | Two places for private material, and one of them is committed. |
| The rule binds **what a person navigates**, not the files inside one program | **ACCEPTED** | A program is already one entry. Its own files are the unit of work for whoever edits it. A language with a flat package convention should not be bent to a folder count. | Either source is buried under folders that exist only to satisfy a count, or the rule is quietly abandoned everywhere. |
