...
fields arrives at 36, and the
table below answers for 36. The two numbers are a criterion, so the next
...
- The number the walk arrives at and the number the table answers for are the same number, derived at check time rather than read off the detail, and the check refuses when the walk arrives at none. It is 36 and the detail says so beside the table
