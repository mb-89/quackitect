...
slice-of-struct fields, exported map-of-string values and exported slice-of-string
fields arrives at 37, and the
table below answers for 37. A MAP COUNTS ONCE, not once for its key and once
for its value, so the walk and the table count the same things. The two numbers are a criterion, so the next
...
instead of reading the word every.
    ...
    a map's key          one line by design, and it is not a heading: a newline
                         in it is cut and its tail moves into the value, and the
                         middle dot is safe there because readBody strips the
                         lead and takes the rest of the heading whole
    ...
    in the frontmatter   Token.ID, Token.Title, Token.Status, Token.Assignee,
                         ...
                         and the four exported slices of string the frontmatter
                         writes as lists: Token.Subs, Token.DependsOn,
...
- The walk starts from Token and follows what the record writes: exported string fields, and into exported struct fields, exported slice-of-struct fields, exported map-of-string values, counted once each rather than once for a key and once for a value, and exported slice-of-string fields, so Criterion, Rejection, Lesson, Token.Submission and Token.Rewatched are all reached because the record reaches them. Every string field the walk arrives at must appear in the shape table, and one the table does not answer for makes it red by name. A deliberate exclusion is written into the table with its answer, so a reader can tell one from an oversight
...
- A field that is one line by design carrying a newline is refused when the token is saved, and the refusal names the field and which criterion it is on. THE SET INCLUDES A MAP'S KEYS, because a newline in one is cut and its tail moves into the value, and the refusal names which map and which key
...
- EVERY value the note joins into a heading is refused when it carries the heading separator, which is the middle dot, or a newline, and the refusal names the field and the character. The set is the one the table names under into a heading, and the check takes it from there rather than asserting it of the clause alone. A MAP'S KEY IS NOT IN THAT SET, and the reason is written beside it: readBody strips the evidence lead and takes the rest of the heading whole, so the middle dot is not a separator there and refusing it would refuse a character the record carries
...
- The number the walk arrives at and the number the table answers for are the same number, derived at check time rather than read off the detail, and the check refuses when the walk arrives at none. It is 37 and the detail says so beside the table, and a map counts once
...
## finding 12 · round 7 · detail, the table under THE FIVE SHAPES, AND WHICH FIELD IS WHICH: "one line by design Criterion.Says, Criterion.Runs, Criterion.Without, Criterion.Red, Token.GuidanceRef" / done when, criterion 7: "The number the walk arrives at and the number the table answers for are the same number" · by reviewer11

**wrong:** TOKEN.GUIDANCEREF IS FILED UNDER ONE LINE BY DESIGN AND THE PARSER TREATS IT AS A BLOCK, SO THE ROW GETS BOTH HALVES WRONG. The table gained the row this round, which answers what I asked for last round, and the row it gained is the wrong one. I measured the field rather than reading the page: in a copy of src/engine outside this tree I saved a GuidanceRef of doc/guidance/a.md, a newline, doc/guidance/b.md, and it came back byte-identical, and I saved one reading doc/a.md, a blank line, a line of two hashes and the word detail, a blank line, more, and it came back as doc/a.md alone. A newline survives and a section opener truncates, which is the signature of Token.Detail and Token.Guidance rather than of Criterion.Says. THE REASON IS IN store.go AND IT IS THE SAME READER: body() writes the field as the guidance heading, a blank line, See and the value, and readBody takes the whole section back and strips the See prefix, so the value is read to the next heading exactly as a detail is. Nothing about it stops at the first newline. THE ROW COSTS BOTH WAYS. Under one line by design, criterion 2 refuses a newline in it, so the record refuses a value it can hold and whoever wrote it loses a save and learns a rule that is not true. And because the row is not a block, criterion 8 takes its set from the table's block row and never reaches it, so the loss GuidanceRef really has, a line that opens a section, is guarded by nothing on this list. THE COUNT DID NOT CATCH IT AND CANNOT. Criterion 7 holds the walk's 36 against the table's 36, and I ran the walk and both numbers are right, so the set is derived and complete. A count of rows says nothing about a row being the wrong one, and this is the only field on the table whose row I could check by running it and find wrong, which is one of thirty-six and is the one the last two rounds have been about.

**satisfies:** MOVE Token.GuidanceRef TO A BLOCK BY DESIGN AND SAY WHAT WAS RUN TO PUT IT THERE. Its reader is readBody and its writer is body(), the same pair that carry Token.Detail, so it belongs beside them, criterion 8 then refuses a line that opens a section in it, and criterion 2 stops refusing a newline the record holds. Then write every other row from a run rather than from a reading: for each of the thirty-six the walk arrives at, feed it the value its row says it cannot hold, save, load, and put what came back beside the row. That is one loop over the set the walk already derives. THE CHECK, RED TODAY, AND IT IS THE SIBLING CRITERION 7 NEEDS: one case per row, feeding the field what the row forbids and requiring what the row promises, so a member under the wrong shape goes red by name. Against the tree as it stands it is red on Token.GuidanceRef twice, once because a two-line value comes back whole where the one-line row says it is refused, and once because a value opening a section comes back truncated where nothing on the list guards it. Record both by the test name and what it said.

## finding 13 · round 7 · detail: "A ROW FOR THE KIND AND NOT FOR THE MEMBER ... every map of string the note writes as a body section" / done when, criterion 4: "EVERY value the note joins into a heading is refused when it carries the heading separator ... The set is the one the table names under into a heading" · by reviewer11

**wrong:** THE MAP KEY IS STILL ANSWERED NOWHERE, AND I MEASURED WHICH HALF OF IT IS REAL. Round 6 said the record writes a map's key into a heading line and its value into a block, and the table decides one of the two. The word key does not appear anywhere in the detail. The block row still reads every map of string the note writes as a body section, which is the value. The into a heading row still names Rejection.Clause, Rejection.By and Lesson.By, and criterion 4 takes its set from that row, so no criterion on this list reaches a key. I DROVE THREE KEYS THROUGH THE RECORD IN A COPY OF THE PACKAGE. A Submission key of two lines is saved with no complaint and comes back as its first line, with the rest of the key moved into the value: sent two, a newline, lines, and read back as the key two holding lines, a blank line, and the value. That is a silent loss on a save, which is the sentence this token was minted for. AND HALF OF ROUND 6's ASK WOULD HAVE BEEN A FALSE REFUSAL, WHICH IS WHY THIS NEEDS SAYING RATHER THAN REPEATING. Its satisfies asked for the refusal to cover a key carrying the heading separator or a newline. I saved a Submission key of what was built, a space, a middle dot, a space and round 2, and it came back byte-identical, because readBody takes an evidence heading whole and strips the lead rather than splitting it on the middle dot the way readFinding and readLesson do. A key carrying two hashes and the words done when also came back whole. So the middle dot is safe in a map key and the newline is not, and a row written from the last finding rather than from a run would refuse a character the record carries.

**satisfies:** GIVE THE MAP ROW BOTH HALVES, WRITTEN FROM WHAT THE RUN ANSWERED. Say: every map of string the note writes as a body section, whose value is a block by design and whose key is one line by design because a newline in it is truncated and its tail moves into the value, which is Token.Submission and Token.Rewatched today. Then say in criterion 2 that its set includes those keys, so a key carrying a newline is refused with a message naming which map and which key, and say in criterion 4 that a key is NOT in its set, with the reason: readBody strips the evidence lead and takes the rest of the heading whole, so the middle dot is not a separator there. That distinction is the row, and writing it down is what stops the next reader refusing a character the record holds. SAY IN CRITERION 1 WHETHER THE WALK YIELDS ONE NAME FOR A MAP OR TWO, because criterion 7 holds the walk's count against the table's and the two have to be counting the same things: the walk arrives at 36 today with each map counted once, and a table answering for a key and a value separately answers for 38. THE CHECK, RED TODAY: put a newline into a Submission key, save, and require the refusal to name the map and the key. Against the tree as it stands it saves without complaint and reads back short, which is the red, and take it once for Token.Submission and once for Token.Rewatched rather than once for the criterion.

## finding 14 · round 8 · detail, the table under THE FIVE SHAPES, AND WHICH FIELD IS WHICH, the row "one line by design ... Criterion.Red, Token.GuidanceRef" / done when, criterion 2 "A field that is one line by design carrying a newline is refused when the token is saved" / done when, criterion 8 "It is refused on every block the table names" · by reviewer12

**wrong:** TOKEN.GUIDANCEREF IS STILL FILED UNDER ONE LINE BY DESIGN, AND THE PARSER GIVES IT THE BLOCK SHAPE. Round 7 finding 12 said so, and this redraft left the row byte for byte as it was. The detail says nothing anywhere about why it should stand. I ran the row rather than reading it, and I ran the whole row rather than the one field the last finding measured. I copied src/engine into a scratch package outside this tree and took the linesThatFit refusal out of SaveToken. Then I drove a two-line value through all five members the row names. Criterion.Says came back as its first line. Criterion.Without and Criterion.Red came back the same way. Criterion.Runs came back empty, because the closing backtick sits on a later line. Token.GuidanceRef came back byte-identical, first line, newline, second line. Four of the five lose the second line and the fifth does not. THE READER IS THE ONE TOKEN.DETAIL USES. body() writes the guidance heading, a blank line, then See and the value. readBody hands the whole section back through sections() and strips the See prefix. It stops at the next heading and never at a newline. THE ROW COSTS BOTH WAYS. Criterion 2 would make the engine refuse a newline the record carries today, so a writer loses a save to a rule that is not true. Criterion 8 takes its set from the block row, so it never reaches this field. AND THE LOSS THIS FIELD REALLY HAS IS WORSE THAN THE TRUNCATION ROUND 7 RECORDED. I saved a token whose Detail was the real detail and whose GuidanceRef was doc/guidance/one.md, a newline, two hashes and the word detail, a newline, the stolen detail. GuidanceRef came back as doc/guidance/one.md, and Detail came back as the stolen detail. The value writes a second detail section inside the guidance section, and readBody keeps the later one. So a section opener in this field overwrites Token.Detail rather than only losing its own tail. THE EXTENT IS ONE ROW MEMBER OUT OF THIRTY-SEVEN, AND I MEASURED THAT RATHER THAN ASSUMING IT. The same loop over the other rows agreed with the table every time. Rejection.By, Rejection.Clause and Lesson.By each corrupt on the middle dot, and Rejection.By moves its tail into the clause. A Submission key and a Rewatched key each hold the middle dot whole and each cut on a newline, with the tail moved into the value. A Reason of two lines, one opening a section, one carrying a fence and one carrying a key all came back byte-identical, with Title, Assignee and Detail untouched. The walk arrives at 37 and the table answers for 37, and I re-derived both numbers, and both are right.

**satisfies:** MOVE Token.GuidanceRef TO A BLOCK BY DESIGN AND SAY IN THE DETAIL WHAT WAS RUN TO PUT IT THERE. Its writer is body() and its reader is readBody, which is the pair that carries Token.Detail and Token.Guidance. The count stays 37, because the field moves between two rows rather than joining the table. Criterion 2 then stops refusing a newline the record holds. Criterion 8 then reaches it, because criterion 8 takes its set from the block row. ADD THE CASE THAT DECIDES IT TO CRITERION 8, AND ASSERT ON THE NEIGHBOUR RATHER THAN ON THE FIELD. A GuidanceRef carrying a line that opens a section does not truncate quietly, it replaces Token.Detail. So the case saves a token holding a detail and a GuidanceRef that opens a section, and requires the refusal by name and the detail unchanged. A case that only asked whether GuidanceRef came back short would pass on a save that ate the detail. WRITE THE REMAINING ROWS FROM THE SAME LOOP AND PUT WHAT CAME BACK BESIDE EACH ROW. The paragraph above is that answer for the one-line row, the heading row, the map key row and the frontmatter row, so this is a re-run rather than a derivation. THE CHECK, RED TODAY, AND IT IS THE SIBLING CRITERION 7 NEEDS. One case per table row, feeding each field what its row forbids and requiring what its row promises, failing by the field name. Against the tree as it stands it is red on Token.GuidanceRef twice. A two-line value comes back whole where the row says it is refused. A value opening a section takes Token.Detail with it where nothing on this list guards it. Record both by the test name and what it said.
...
**the class:** A TABLE WHOSE MEMBERS ARE DERIVED AND WHOSE ANSWERS ARE STILL GUESSED. A round asks for the set to be derived rather than typed, and the next draft does it: the walk runs, the count is in the note, and the two numbers agree, so the table stops being a photograph and every member is there. Then each member is given a row by reading its name and where it sits on the page, and no row is run. The check that holds the two counts against each other cannot see a wrong answer, because it counts rows rather than trying them, and the answers are what the work will be built from. A WRONG ROW COSTS BOTH WAYS: a field under a stricter shape than it has gets a refusal for a value the record can hold, and a field under a looser one keeps the loss the table was written to end, and both read as decided. THE TELL is to ask of each row what was run to put the field there, because a shape is a fact about the parser and not about how the value looks when a person types it. MEASURED ON wk-24be1c06ae round 6: the walk arrives at 36 and the table answers for 36, both right, and Token.GuidanceRef is filed under one line by design while a two-line value comes back byte-identical and one opening a section comes back truncated, which is the block signature.

**instead:** WHAT WOULD HAVE STOPPED IT BEING MADE: write each row from a run rather than from a reading. For every member the walk arrives at, feed it the value its candidate row says it cannot hold, save, load, and put what came back beside the row. That is one loop over the set the walk already derives and it costs less than the round a wrong row buys. Where a member is written to the page in more than one place, a map key and its value, or a heading and the block under it, it needs an answer for each place rather than one for the field. WHAT WOULD HAVE CAUGHT IT: two counts that agree prove the table is the right size and say nothing about it being right, so the check that holds them needs a sibling, one case per row feeding the field what the row forbids and requiring what the row promises, so a member under the wrong shape goes red by name. A row nothing has tried is a decision nobody made.

**minted as:** wk-5697d58b1b

## lesson 8 · round 8 · by reviewer12

**the class:** A REDRAFT THAT ANSWERS ONE OF A ROUND'S FINDINGS IN FULL AND LEAVES THE OTHER BYTE-IDENTICAL, WHICH IS wk-b062e097c9 NAMED AGAIN AND WIDENED. That class says the finding left behind is the one whose remedy is a decision, and its tell is a satisfies offering two routes. On wk-24be1c06ae round 7 neither finding offered a route, so that tell does not fire. Finding 13 asked for the map row in two halves and got it, in the detail and in criteria 1, 2 and 4. Finding 12 asked for one name to move between two rows of the same table, after a run. The map row was rewritten and the GuidanceRef row came back byte for byte, with no sentence saying it was considered. So the sorting rule is not an edit against a decision. It is cheap against measured. Finding 13 could be answered by copying the paragraph the finding had already written. Finding 12 needed the drafter to run the field first, and running a field is the step an edit pass has no room for. The redraft reads as productive either way, because the diff touches every criterion one finding named.

**instead:** Before submitting a redraft, put every criterion and every paragraph the round's findings quoted beside what it says now, and list the ones that came back byte-identical. Take that list from the note's git diff rather than from memory, because the diff is what the next reviewer reads. Any name on the list is unanswered until the draft says in words why it stands as it is.

**before it:** Answer a round one finding at a time, and take the one that needs a command run first, while nothing has been typed. Where a finding asks what a field does, run the field before touching the page. Then the row is written from what came back rather than from what the name looks like.
