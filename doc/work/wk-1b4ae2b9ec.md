...
| [x] | the change follows the approach on the token, or the token says why it departed | AStaffShortfall takes the id and the disposition. A submit is read before every other hold. Both doors read one rule. |  |
| [x] | se test --on this token answered ok, and what it ran is named | TestAShortfallLetsASubmitThrough: ok, and it had never run. Twelve more went green beside it. |  |
| [x] | the note says what changed and why, for a reader who was not here | staffing.go carries aSubmit and the wider signature. hook.go carries the two fields. Sixteen call sites updated. |  |
