...
|---|---|---|---|
| [x] | the approach is on the token before any work, as an interface or a shape a reader can disagree with | the proposed action names the interface: --from PATH on se run and se apply, read whole, under .se/scratchpad only. | the proposed action, above |
| [x] | every done-when line is decidable, and names the command where one decides it | all seven name a command. Six were run and answered. The seventh cannot fail, because nothing changed. | below, one row each |
| [x] | the change is small enough to review whole, or it is split first | two flags, one refusal rewritten, one check with the Go test under it. | `git diff fc27d9b1^ fc27d9b1` |
| [x] | the basics it stands on exist, or are minted first | insideTheScratchpad and runsTheEngine both existed. Nothing was minted. | src/engine/gate.go |
...
| done | criterion | evidence | receipt |
|---|---|---|---|
| [x] | the guidance this token names was read and applied | read. Every criterion was run, not read off the source. | doc/guidance/work-token.md |
| [x] | the change follows the approach on the token, or the token says why it departed | it does, and all of it was in the tree at began. fc27d9b1 carries both flags, theRefusal and the check. | `git log -1 -- src/engine/gate.go` |
| [x] | se test --on this token answered ok, and what it ran is named | ok, on both checks the token names. | a-refusal-names-a-legal-move, refusals-name-a-door |
| [x] | the note says what changed and why, for a reader who was not here | run --from answered exit 0 on a file holding ls, apply --dry --from answered ok, and a path climbing out was refused. Put the piped line back on a scratch copy and the check reddens, naming it. | the commit message |
| [x] | the cleanup the change revealed is in the change, or is a token of its own | none. Nothing changed here. | — |
