...
| **Measured, and the number is the owner's** | **SPIKE**, wk-23801b6603 | Every figure has the command that answers it, because a number in a design document with nothing behind it outlives everybody who could have checked it. `uvx --from tiktoken python util/checks/count-standing.py .` answers what one turn carries, in cl100k_base, which is not this host's own encoding and is the nearest reproducible one. The number itself is not written here: a count in prose is stale the day after it is measured, and the command is what stays true. The ceiling put to the owner: 6,000, and a rule added above it is paid for by one removed. What still owes a number is the other half, whether a rule in this layer changes behaviour at all, which is wk-69150c6d69 and is the half that should decide the ceiling. | A budget nobody set is a budget nobody meets. |
