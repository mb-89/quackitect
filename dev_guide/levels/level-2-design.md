...
## The retro that promotes a helper script

An agent may write a helper script for anything it does more than once, and it writes it
into `scratchpad/`. Most of those are specific enough that nobody reuses them. A few earn
a place in `util/scripts/`, and the retro is what moves them. Promotion is a decision with
a cost, so it belongs to a person or to a token, never to the agent that wrote the script.

| Entry | Ruling | Why | Breaks without it |
|---|---|---|---|
| **A promoted script answers `--help`** | **ACCEPTED** | It is what every program here owes a reader, and the catalogue is built from it, so one convention serves a person and the engine at once. | Either the catalogue is authored by hand beside the script and drifts from it, or the person who finds the script six months later has to read it to learn what it takes. |
| **The catalogue is derived, never authored** | **ACCEPTED** | One owner per fact. The script decides what it does, so the script is what says so. | A manifest that says something the script stopped doing, with nothing to notice. |
| **`--help` is re-run only when the script changes** | **ACCEPTED** | The digest rule this project already uses for reprojection, applied to the same problem. | Either the catalogue costs a process per script at every boot, or it is cached and goes stale. |
| **A promotion without `--help` is refused** | **ACCEPTED** | Mechanical, so the engine checks it. A rule that is only written down is a rule that decays. | The convention holds until the first hurried promotion, and then it holds for nothing. |

The catalogue reaches the agent the way the machine's tool list does, on the first pull of
a session. It costs one line per script, once. It is never an MCP tool per script: a tool's
description sits in the prompt on every turn of every session, whether or not anybody calls
it, and that is what made v3's tool list expensive.

**Level 0 discovers, Level 1 delivers, Level 2 promotes.** Reading a folder and running
`--help` is mechanical. Handing the list over is the pull. Deciding that a script earned
its place is a judgement, and judgement is why this entry sits here.

## Leaving a state with work in review

Level 1 refuses a stop while an actor holds work it could still do, and it counts submitted
work as out of that actor's hands. That is right for stopping, and it is wrong for moving.

| Entry | Ruling | Why | Breaks without it |
|---|---|---|---|
| **A state cannot be left while work in it is submitted** | **ACCEPTED** | A submission is work that is finished but not judged. Moving on treats a verdict nobody gave as an acceptance. | The walk advances on work that a reviewer later rejects, and the rejection lands in a state that has already been left. |
| **A submitted token does not hold the worker from stopping** | **ACCEPTED, and it stays at Level 1** | Stopping and moving are different acts. Nothing the worker does releases a submission, so refusing its stop would trap it waiting on somebody else. | Either the worker is held on a queue it cannot empty, or the state moves on unjudged work. One rule cannot serve both. |

The condition is a set of Level 1 tokens, which is what every Level 2 condition is. Nothing
new is needed for it beyond the states themselves.
