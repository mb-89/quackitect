...
| Run | `sh util/checks/battery.sh`, which runs `se --selftest` and `go test ./...` in each source folder |

**This map stops at Level 0.** Level 1's tokens and pull are tested by `go test` in
`src/engine`, and the work they carry lives under `.se/work/`, which is private. So Level 1
has no row here. Say so plainly, because a reader who finds a coverage map reads it as the
whole picture.
...
| A cage that travels names no machine | `TestNoCagedFileNamesTheMachineItWasWrittenOn`, `TestNoCommittedProjectionNamesTheMachine` |
| One program under two names, and when that stops being true | `TestTwoNamesAreOneFileUntilSomebodyBuildsByHand` |
