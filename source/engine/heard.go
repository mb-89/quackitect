...
func CopyWhatWasHeard(r Roots, transcript string, log *Log, actor string) int {
	...
	for in.Scan() {
		...
		// ONE PROMPT, ONE RECORD. The agent's said verb writes the same
		// messages, and whichever gets there first is the one that stands.
		if AlreadySaid(r, said) {
			continue
		}
		log.Write("user", "prompt", "owner", said, nil, map[string]any{"heard": "mid-turn"})
		// THE AGENT THAT HEARD IT IS THE ONE THAT OWES AN ANSWER. Several run
		// here at once, and one message blocking all of them drew three answers
		// to one question.
		_ = TheyAsked(r, actor, said)
		copied++
	}
