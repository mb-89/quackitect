...
by: agent
signed_off: 2026-08-20T11:00:58.084Z
authors: agent
files:
---
...
- affordable: SEVEN OF THE EIGHT ARE SMALL AND ONE IS NOT. Six are a field, a lookup, a refusal or an assertion — each is hours. THE EXPENSIVE ONE IS req-every-call-records-the-state-it-was-made-in, not because the field is hard but because the value must be stamped where the call is served for every verb, and the retro has carried its absence as a documented impossibility since 2026-08-17. AND ONE IS AFFORDABLE ONLY IN THE NARROW SENSE: req-every-call-records-the-model-that-answered-it is cheap to build and its VALUE waits on the model coming from somewhere other than the agent, which is an open assumption rather than work this iteration can price.
