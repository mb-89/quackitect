...
form: specify-build
by: agent
signed_off: 2026-08-17T12:01:45.746Z
authors: agent
files: null
---
...
One design spec is authored below the architectural line: dsp-the-arrival, realizing el-arrival.

It names three files — the act, the hook that fires it, and the committed root settings that is the only file a fresh clone reads at session start.

No spikes were run, so there are no promotions to assign.
...
- The spec deliberately excludes a shared module with se-start.ts, which leaves four functions implemented twice. That is the repayment written on raid-iss-two-entrypoints-place-the-cage-and-nothing-compares-them.
- The build is already largely done, because the arrival was built under an owner instruction before the walk reached this state. build-steps records that honestly rather than pretending the order was clean.

## anything_else

THE ORDER OF THE SIX STEPS IS THE ONLY REAL DESIGN DECISION HERE, and two of the six placements are load-bearing.

RUNTIME BEFORE INSTALL AND BEFORE ANY SPAWN. Everything below it runs node <file>.ts unflagged, so a wrong runtime turns every later failure into a syntax error deep inside a spawned script — which is precisely the confusing failure cloud-runner.md already warns about.

CAGE AFTER RUNTIME AND NEVER BEFORE. A cage placed beside a lane that never came up is the silent half-arrival this element exists to prevent: the deny list is on disk, so everything looks arrived, and there is nothing to work through.

THE STEP ORDER IS PINNED BY A TEST rather than left as prose. tests/arrival.test.ts asserts that an arrival stopping at the runtime step places no cage, which is the ordering claim stated as an observable.

NO PROMOTIONS, and the table is empty for a real reason: M6 is struck at minor, so no spike ran. An empty promotions table at this size is the expected shape, not an omission.
