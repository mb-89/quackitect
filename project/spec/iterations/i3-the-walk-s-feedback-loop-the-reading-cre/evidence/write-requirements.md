...
- complete: every step this delta touches has a covering row. The reading credit refines uc-be-handed-the-method; the red objective and the grey verb both refine uc-take-a-step. The resident register covers everything this delta does not touch.
- consistent: no two rows conflict. The red-objective row narrows one case of the wait and says so in its Detail, so it does not contradict general wait behaviour. Credit, objective and condition each mean one thing across all three rows.
- affordable: three rows, all verified by test, all against machinery that already exists. No new harness is needed, and the paired notes carry the mechanism to a file and a line.
- bounded: every row sits inside the blessed scope and answers to a note recorded when the defect was met. The introspection verb is the only new capability, and the version plan places it here explicitly.
- comprehensible: each statement is one EARS sentence under twenty words naming an outcome rather than a mechanism, so a reader from any discipline can say what the system must do from the set alone.
- no_tbd: swept for TBD, TBC, TBR and ??? across all three files. Zero found, and the sweep was run rather than assumed.
- behaviour_modelled: the look was taken on all three and one earned a model. req-reading-credit-survives-a-reload carries a lifecycle, because the defect IS a missing transition and that is invisible in prose. The other two say in one line why they have none, being one condition and one response, and one question and one answer. A model on either would restate the statement in a second notation.

## follow_up

- derive the functions and identify the assumptions these three rows lean on
- keep the grey-state verb's surface distinct from se.help, per raid-asm-grey-verb-distinct-from-se-help
- key the reading credit to content rather than to a path, which discharges raid-dep-reading-credit-outlives-se-move
- author the tests before the build, so the three reds are observed rather than assumed
