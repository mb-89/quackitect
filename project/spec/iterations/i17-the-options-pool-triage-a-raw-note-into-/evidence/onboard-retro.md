...
- note-747e0ee09da0 prose-inspect was permanently red and boot could not finish: carried. Fixed in engine/bin/prose-inspect.ts. The collision guard now also mutes a BARE word the records already use in more than three files and prints it as a blind spot, and matching is word-boundary aware so HOME=/root no longer matches tests/roots.test.ts. Five cases in tests/identity-collision.test.ts pin it, three of which prove a real leak still fires. The boot deadlock it also named is NOT closed and is carried into the improvements below.
