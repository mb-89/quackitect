...
- Harvest v1's 25 working .base query files from `spec/queries/` at ref `main`, plus the reader ADR at `spec/decisions/adr-query-in-engine.md`.: SERVED — all 26 files harvested and shipped inside the archive at project/spec/queries/ and project/spec/decisions/adr-query-in-engine.md.
