...
form: author-tests
by: agent
signed_off: 2026-08-16T16:46:16.261Z
authors: agent
files:
---
...
FOUR NEW TEST-SPEC NODES, NINETEEN CASES, NINE REQUIREMENTS. Every case is authored before the build so each is watched failing.

ONE REQUIREMENT WAS WRITTEN HERE, and it is the row gate-requirements named as owed. req-a-value-outside-its-vocabulary-refuses covers the case req-a-write-that-breaks-the-corpus-refuses explicitly sends away — a node that PARSES and carries a word its key does not allow.

THE OWNER RULED IT IN, 2026-08-16: the trapped-walk fix belongs in this iteration's implementation.

THE TABLE ABOVE IS COMPUTED, and then typed back. See anything_else — that round trip is the fourth live instance of this iteration's own subject.
...
WHAT IT WILL FIND, predicted here so the prediction can be wrong on the record.

- ALL NINETEEN CASES RED. Nothing they assert exists yet.
- MOST RED FOR THE RIGHT REASON — an assertion failing on a value. The lane answers, the response is not what the case demands.
- TWO SPECS MAY BE CRASH-RED RATHER THAN ASSERTION-RED. tsp-bound-rules and tsp-coverage-computes-both-sides drive shapes that do not exist — a `rules:` key in frontmatter, and `findings` on se_lint's answer. Those may throw rather than assert.

THAT DISTINCTION IS THIS ITERATION'S OWN SCOPE. Assertion-red versus crash-red is readable from TAP, and building it is item eight of fifteen.

THEN THE BUILD, in the order raid-risk-the-small-fixes-crowd-out-the-conformance-system makes binding.

- The write-budget measurement first, on the cheapest check.
- The conformance chunks before the fixes.
- gate-implementation checking the order rather than the count.

NOTHING IS BLOCKED.

## anything_else

### The computed table, typed back — a fourth instance

THE FORM SERVED THIS FIELD ALREADY FILLED. Fifty-five rows, every method and every verifies edge, all computed from the corpus. Nothing in it came from me.

THE SUBMIT THEN REFUSED IT AS EMPTY, and the fix was to echo the engine's own output back at the engine.

SO A PREFILL IS SERVED AND NOT PERSISTED. The computation is right, the round trip is waste, and req-a-coverage-check-computes-both-sides names the general form: where the corpus can answer, asking the agent turns a computation into a transcription.

THIS ONE IS CHEAPER TO FIX THAN THE OTHER THREE. The engine already has the answer at serve time. Nothing has to be computed that is not computed today.

### What the fixtures guess, said plainly so nobody reads them as the spec

TWO SHAPES ARE INVENTED BY THE CASES because the build has not chosen them.

- A `rules:` list in a node's frontmatter, with key, allows, binds and on_break. tsp-bound-rules says so in its own What-the-cases-assume section.
- `findings` as an array on se_lint's answer.

THE REQUIREMENTS DEMAND NEITHER. They demand that a rule binds to a named node without engine code, and that a corpus-wide check reports. If the build picks other names, these cases change with it and the claims do not.

WRITING THAT DOWN IS THE POINT. A fixture read as a specification is how an implementation detail becomes a requirement nobody agreed to.

### One case asserts an absence, and that is the hard kind

tsp-coverage-computes-both-sides' first case demands that NO coverage field still asks the agent to enumerate what the corpus holds. It matches the served form against the defect's own wording.

A TEST FOR AN ABSENCE IS WEAK BY NATURE. It can only see the phrasings it knows to look for.

SO IT IS PAIRED WITH TWO POSITIVE CASES: a correct corpus passes with nothing listed, and a genuine hole still refuses by id. The absence case guards the wording; the pair guards the behaviour.

### Where the existing battery was read rather than claimed

THE LAW SAYS MAP HONESTLY — an existing file may already carry a requirement, and a mapping nobody checked is fabricated coverage.

NONE OF THE NINE ROWS IS CARRIED BY A STANDING FILE. All nine are new demands minted this iteration, so no existing case could assert them. The four new files are the whole of their coverage, and nothing was claimed from a battery file that was not read.
