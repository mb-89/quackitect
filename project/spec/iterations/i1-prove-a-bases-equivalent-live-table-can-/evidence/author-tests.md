---
form: author-tests
amended: "2026-08-11T13:04:39.447Z by agent — the M8 recut authored two demonstration specs for the must stories after the claim signed - the checks table gains their rows and the counts move from 29 to 31"
by: agent
...
The backfill landed (owner ruling 2026-08-11): 2 new stories, 3 new use cases and 20 new requirements reverse-engineered from the orphan tests — the live table, the autonomy surface, the design-check machinery and the panel's watching contract. 4 new specs and 6 extended file lists claim them. The M8 recut (killer → MoSCoW, same day) added two demonstration specs for the must stories — tsp-hand-walk and tsp-reading-proof-run, each carrying its demonstrates: edge — and demonstrates: edges on three standing demonstration specs. 31 specs cover all 170 requirements, every one of the 91 test files is referenced by at least one spec, and the law, the conformance and the both-way coverage all answer zero.
...
| [[tsp-hand-walk]] | demonstration | none — demonstrates: sty-walk-it-by-hand carries the edge; the mechanics are test-verified by tsp-walk-discipline | none — the procedure below is the definition; the observed session is the evidence |
...
| [[tsp-reading-proof-run]] | demonstration | none — demonstrates: sty-the-agent-proves-it-read carries the edge; the mechanics are test-verified by tsp-reading-loop | none — the procedure below is the definition; the observed session is the evidence |
