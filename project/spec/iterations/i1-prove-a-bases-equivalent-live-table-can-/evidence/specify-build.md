...
form: specify-build
by: agent
signed_off: 2026-08-11T10:06:11.163Z
authors: agent
files:
---
...
The design below the architectural line is specified: 17 design-spec nodes partition every engine file across the 9 elements and 16 interfaces. The one promotion (exp-trunk-read-cost) is assigned to the trunk-batch-reader chunk, seeded in the drawing. The strategy lenses that shaped the order: the build already stands — the specs are reverse-engineered at file grain, and the parallel-flow lens shaped the spec partition itself (one strand per element).

## design_specs

| design-spec | realizes | files |
| --- | --- | --- |
| [[dsp-boot-and-power]] | el-bootstrap | project/deliverable/engine/bin/bench-boot.ts · project/deliverable/engine/bin/se-pty.ts · project/deliverable/engine/bin/se-hook-stop.ts |
...
| [[dsp-form-editors]] | el-mirror | project/deliverable/engine/editors (19 files, one per editor plus the registry) |
...
| --- | --- | --- |
| [[exp-trunk-read-cost]] | the long-lived batch reader as the M7 build's trunk-read shape — a spawn per read is ruled out by measurement | trunk-batch-reader |

## follow_up

trace-design runs the mechanical sweep next: file existence and the unclaimed list. Dead code found there is removed or honestly specified, per the owner's standing order.
