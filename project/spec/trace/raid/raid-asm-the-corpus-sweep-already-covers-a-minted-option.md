...
`project/spec/trace/option/`, with a git identity and a HOME belonging to
nobody real, and the option's body carrying both of them. prose-inspect was run
against that root.

    prose-inspect RED - 2 finding(s)
    - item 3 - project/spec/trace/option/opt-a-minted-option.md:6 - carries the git user name
    - item 3 - project/spec/trace/option/opt-a-minted-option.md:6 - carries the home directory
    EXIT=1
...
`prose-inspect.ts` walks `join(root, "project", "spec")` recursively over every
`.md` file. A node under `project/spec/trace/option/` is inside that walk by
construction.
