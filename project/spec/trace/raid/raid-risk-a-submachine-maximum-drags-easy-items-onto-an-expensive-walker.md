...
how_likely: expected
probe: "PARTLY MEASURED AT M4, 2026-08-20, and the first data cuts against the worst case. THE TWO SUBMACHINES THIS RECORD WALKED WERE HOMOGENEOUS. enumerate-space fanned seven finder states and one chart state; FIVE of the seven take an identical shape — current_situation, applies, one table, options, follow_up — while find_prior_art asks three extra fields and find_by_probing one, so the spread across the fan is one field for one state and three for another against a floor of five. Near zero, not zero, and a maximum over them costs the difference between five fields and eight. run-candidates fanned four compose states with one identical form; the spread is exactly zero. Where the fan is a repetition of one kind of work, this risk does not bite. WHERE IT WILL BITE IS THE BUILD FAN, and that is untested: M7 build-steps deals one chunk per unit of work, and chunks are heterogeneous by construction. THE PROXY ACROSS THE WHOLE MATRIX IS WIDE, which is the other half: at major the 53 rows spread over seven field-count classes, from 10 rows asking no evidence field to one asking six. So the spread exists in the population and was absent in both fans measured. That is a real distinction and the design should turn on it: a maximum over a homogeneous fan is free, and over a heterogeneous one it is the multiplier this entry describes. THE ORIGINAL NOTE, KEPT: NOT YET MEASURABLE, and that is the honest state of it — no submachine has been walked under a rated matrix because no matrix is rated. THE ARITHMETIC IS NOT IN DOUBT: the rule is the maximum over items and one walker walks all of them, so the multiplier is the item count whenever the items differ. WHAT IS UNKNOWN is how often they differ and by how much, and the per-item values this iteration produces are exactly the data that answers it. Re-read this entry once the ratings land, with the item spread per submachine beside it."
probed: 2026-08-20
source_refs:
