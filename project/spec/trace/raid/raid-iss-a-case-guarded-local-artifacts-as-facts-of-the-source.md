...
- `project/scratchpad` is the workbench and is never committed.
- `project/.vscode` is an editor's own folder.
- `project/.obsidian` IS tracked, and is the only one the guard fits.
...
- The source-side guard stands for `project/.obsidian`, which the repository
  actually tracks.
