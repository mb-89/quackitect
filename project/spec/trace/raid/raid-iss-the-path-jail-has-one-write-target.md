...
`project/deliverable/engine/paths.ts` opens with the rule in its own words.
...
`join(root, "project", "deliverable", "machines", ...)`.
...
resolver in `project/deliverable/engine/paths.ts`, which is where a second
