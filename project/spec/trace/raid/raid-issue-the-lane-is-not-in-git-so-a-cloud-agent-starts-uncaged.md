...
extension into the OPENED folder from project/_cage/ — never committed".
...
answer false: `.mcp.json`, `.claude/settings.json`, `project/.mcp.json` and
`project/.claude/settings.json`.
...
MCP config out of `project/deliverable/cage/`, then spawns a caged subagent
...
`project/guidance/method/cloud-runner.md` under "Arrival A", and an agent with
...
`project/_cage/`. They live in `project/deliverable/cage/`. The comment is
