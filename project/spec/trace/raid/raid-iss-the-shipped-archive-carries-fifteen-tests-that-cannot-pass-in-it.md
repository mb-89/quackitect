...
type: "[[raid]]"
kind: issue
statement: the package ships the test suite and excludes project/spec, so fifteen cases that read this repository's own corpus fail on every fresh install.
owner: the owner
...
- The package excludes `project/spec` deliberately. That folder is the
...
- SKIP RATHER THAN FAIL. Each of the fifteen already knows it needs the
  ...
  `project/spec/trace` is absent turns fifteen reds into fifteen honest skips.
