...
type: "[[raid]]"
kind: risk
statement: Moving the root a path resolves against can send a write to the wrong tree, and a wrong resolution reports success.
owner: the driving agent
trigger: at the first write of each path KIND after the root moves - method, record content, session state and repo-root files each resolve by a different rule
status: open
