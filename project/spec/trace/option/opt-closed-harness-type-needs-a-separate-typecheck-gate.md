...
found_by: probe
source: "Probed 2026-08-19: project/scratchpad/probe-harness-type-exhaustiveness.ts, a discriminated union with one case (\"cursor\") deliberately left unhandled, run with `node <file>.ts` (no flag). Exit 0, stdout \"fell through\" \u2014 the missing case did not error. deliverable/package.json carries no tsc or typecheck script, confirmed by search."
---
