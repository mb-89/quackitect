...
except the check that stops before a partial install — and the prior-art
sweep found no account of anybody doing that either, so it is the one part
with no environment equivalent to lean on.
