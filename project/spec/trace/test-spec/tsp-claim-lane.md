...
id: tsp-claim-lane
type: "[[test-spec]]"
statement: A seeded iteration reaches the remote, a claim is one atomic pushed file that races first-push-wins, wears its age, reconciles offline, and releases only by a recorded force — verified by test over the claim lane.
method: "test"
verifies:
...
The claim lane end to end: the seed push, the claim file, the race, the
listing, the offline path and the force release. The lane's privacy and
