...
|||
OBSERVED, 2026-08-19, at the build step `the-version-flag`: `node project/deliverable/engine/bin/se-mcp.ts --version`. One command, no arguments to get right, and it is listed in `--help` beside every other switch, so it is discoverable without being known.
