...
- 2y. THE DRIVEN TREE CARRIES NONE OF THE SYSTEM'S METHOD, which is the ordinary case rather than an error. The system finds its method through a recorded pointer to the copy that made the project, never by looking in the tree it is working on. That is the crossing the path jail refuses today, and it is v1's engine-home mechanism - see note-b966f8fd311e.
