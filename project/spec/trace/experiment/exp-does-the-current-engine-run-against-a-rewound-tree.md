...
Same rewound tree, with `project/deliverable/machines` and `project/guidance`
replaced by today's:
...
| `grep -rl i33` under `project/spec/trace` | **0** |
...
rather than machine. It did not say that `project/guidance` is machine, and
...
- Whether a walk can be DRIVEN to completion in such a tree. One pull is not a
  walk.
- What the rewound `project/spec` does to a state whose entry condition reads
