...
type: "[[experiment]]"
statement: Can el-query-evaluator's declarative AND-filter over frontmatter answer a structured query against the real trace corpus (project/spec/trace) in under one second, measured as wall-clock walk-plus-read-plus-parse-plus-filter time?
probes:
  - raid-risk-i15-query-latency-unmeasured-at-real-scale
timebox: 45 minutes
form: script
chunk: "none"
faked: the filter evaluator itself — a minimal regex-based frontmatter parser and a two-field AND predicate (kind==risk, status==open) stand in for the real filters.and/or evaluator; the corpus walked and read (project/spec/trace, 768 .md files at measurement time) is not faked, and no cache or index sits in front of it
...
- Walk `project/spec/trace` recursively for `*.md` (`fs.readdirSync` with
  `withFileTypes`).
