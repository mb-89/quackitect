...
1. Install the twenty CodeMirror packages into `project/deliverable`. All MIT, no build tooling.
...
4. Register a custom text editor in `project/deliverable/vscode/package.json` and in `extension.js` beside the three existing view registrations. Reuse the existing surface class and shell-page builder unchanged, so neither constraint is touched.
