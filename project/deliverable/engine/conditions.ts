...
export const CONDITION_TYPES: ReadonlySet<string> = new Set(["read", "read_consume", "script", "evidence_form", "no_pending_note"]);

export function conditionNotePath(type: string): string {
  return ["project", "deliverable", "machines", "conditions", `${type}.md`].join("/");
}

export function conditionNoteAbs(root: string, type: string): string {
  return join(root, "project", "deliverable", "machines", "conditions", `${type}.md`);
}
