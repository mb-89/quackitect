...
function baseFile(root: string, rel: string): string {
  ...
  if (!abs.startsWith(resolve(dir))) {
    throw new Rejection({
      ...
      got: rel,
      remedy: { tool: "se_file_list", args: { dir: "project" }, note: "a control may only write a base the vault owns" },
      source: SRC,
    });
  }
  if (!abs.endsWith(".base")) {
    throw new Rejection({
      ...
      got: rel,
      remedy: { tool: "se_file_list", args: { dir: "project" }, note: "the controls edit a base, never a note" },
      source: SRC,
    });
  }
  return abs;
...
function findView(doc: Doc, name: string): ViewDoc {
  ...
  if (hit === undefined) {
    throw new Rejection({
      ...
      got: name,
      remedy: { tool: "se_file_read", args: { path: "project/spec/bases-syntax.md" }, note: "section 6 covers views" },
      source: SRC,
    });
  }
  return hit;
...
export function setSource(root: string, rel: string, text: string): string {
  ...
  try {
    ...
    throw new Rejection({
      ...
      remedy: { tool: "se_file_read", args: { path: "project/spec/bases-syntax.md" }, note: "section 1 gives the five top-level keys" },
      source: SRC,
    });
  }
  if (doc === null || typeof doc !== "object" || Array.isArray(doc)) {
