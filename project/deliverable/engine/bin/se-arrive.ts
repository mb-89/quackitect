...
// and `project/.mcp.json` is not in git — so the lane it is supposed to work
...
function install(): void {
  const deliverable = join(ROOT, "project", "deliverable");
...
function cage(): void {
  const from = join(ROOT, "project", "deliverable", "cage");
  const placed: string[] = [];
  for (const [src, dest] of [
    ["mcp.json", join(ROOT, "project", ".mcp.json")],
    ["claude-settings.json", join(ROOT, "project", ".claude", "settings.json")],
  ] as [string, string][]) {
...
unattended walk is given is project/deliverable/cage/kickoff.txt.
