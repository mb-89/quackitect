...
export const STANDARD_ROUNDS: EvidenceField[] = [
  ...
  // ASKED AT EVERY GATE, SO NOTHING IS DROPPED. A review is where somebody
  // has just looked hard at the work, which is the moment a risk or an
  // assumption is most visible and least likely to be written down. Waiting
  // for the state that owns the register is how an entry is lost. NONE IS A
  // LEGAL ANSWER and most gates will give it; the cost of asking is one line,
  // and the cost of not asking is an assumption nobody records until it
  // breaks.
  {
    name: "raid_additions",
    description:
