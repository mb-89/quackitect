...
THIS STATE'S INPUT IS THE RAID FOLDER, not the state above it. Every standing assumption is probed, whenever it was recorded. That is why identifying and probing are two states (owner ruling, 2026-08-06): with one state doing both, "probe assumptions" naturally reads as "probe the ones I just wrote", and an assumption recorded in i1 is never looked at again — which is exactly when it has most likely gone stale.
