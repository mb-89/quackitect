...
const ELEMENT = "project/spec/trace/element/el-probe-carrying-a-bound-rule.md";
...
test("a rule written into a node fires on the next write, with no engine file touched", async () => {
  ...
  const before = spawnSync("git", ["status", "--porcelain", "--", "project/deliverable/engine"], {
    cwd: root,
    encoding: "utf8",
  }).stdout;
  ...
  const after = spawnSync("git", ["status", "--porcelain", "--", "project/deliverable/engine"], {
    cwd: root,
    encoding: "utf8",
  }).stdout;
...
test("a rule naming a node the corpus does not hold is reported as unbound", async () => {
  ...
  const swept = sweepCorpus(root, "project/spec/trace");
...
test("an unbound rule is told apart from a rule nothing violated", async () => {
  ...
  const findings = sweepCorpus(root, "project/spec/trace").findings;
