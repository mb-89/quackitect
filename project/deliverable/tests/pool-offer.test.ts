...
/** Mint `n` work tokens, then hand back a root whose local note store is EMPTY of
...
function rootWithPool(n: number): string {
  ...
  for (let i = 0; i < n; i++) {
    ...
    drainNote(
      ...
      captured,
      "backlog",
      `ready when thing ${String(i)} is true`,
      true,
      `work token number ${String(i)} stands`,
      root,
    );
  }
  return root;
...
describe("what stands open is read from the repository", { concurrency: true }, () => {
  test("every minted work token is offered", () => {
  ...
  test("a work token says what it is and when it comes back, without opening anything else", () => {
    const root = rootWithPool(1);
    const [item] = survey(root).backlog;
    assert.match(item.title, /work token number 0 stands/, "the offer does not carry the work token's statement");
  ...
  test("an undrained capture is not a work token, and the pending count still reports it", () => {
    ...
    assert.equal(s.counts.backlog, 2, "an unjudged capture was offered as a work token");
    assert.ok(
...
describe("one source, two readers", { concurrency: true }, () => {
  test("the lane's answer and the pool agree on the ids at the same moment", () => {
    ...
    const fromPool = standingTokens(root)
      .map((o: { id: string }) => o.id)
      .sort();
    assert.deepEqual(fromSurvey, fromPool, "the two readers of one pool named different work tokens");
  });

  test("a mint is visible to the next read with no reload", () => {
    const root = rootWithPool(1);
    assert.equal(survey(root).counts.backlog, 1, "the first work token is not offered");
    const { captured } = appendNote(seDir(root), "a second raw capture");
    drainNote(seDir(root), captured, "backlog", "ready when the second thing is true", true, "a second work token stands", root);
    assert.equal(survey(root).counts.backlog, 2, "a freshly minted work token was not offered until something reloaded");
  });
});
