...
describe("the clause-guidance pairing", () => {
  ...
  test("every registry clause has its section in refusals.md", () => {
    ...
    assert.deepEqual(missing, [], "a new clause is not done until its section stands in guidance/refusals.md");
  });
  ...
  test("the rejection payload carries the feed-forward pointer", () => {
    ...
    assert.ok(r.toJSON().guidance.includes("guidance/refusals.md"), "the pointer names the page");
