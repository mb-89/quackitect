...
const ARRIVE = join(REPO_ROOT, "project", "deliverable", "engine", "bin", "se-arrive.ts");
const HOOK = join(REPO_ROOT, "project", "deliverable", "engine", "bin", "se-hook-arrive.ts");
...
function rootWithNoLane(): string {
  ...
  const cage = join(root, "project", "deliverable", "cage");
  ...
  writeFileSync(join(root, "project", "deliverable", "package.json"), JSON.stringify({ engines: { node: ">=999.0.0" } }));
  return root;
}
...
describe("the arrival", { concurrency: true }, () => {
  ...
  test("a runtime below the declared floor stops the arrival, and the declaration is left alone", () => {
    ...
    const before = readFileSync(join(root, "project", "deliverable", "package.json"), "utf8");
    ...
    const after = readFileSync(join(root, "project", "deliverable", "package.json"), "utf8");
  ...
  test("an arrival that stops at the runtime places no cage", () => {
    const root = rootWithNoLane();
    arrive(root);
    assert.equal(existsSync(join(root, "project", ".mcp.json")), false, "no lane config is left behind");
    assert.equal(existsSync(join(root, "project", ".claude", "settings.json")), false, "no deny list is left behind");
  });
