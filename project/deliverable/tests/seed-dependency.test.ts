...
test("an empty list is legal and lands as a stated decision", async () => {
  ...
  const rec = await call(server, "se_file_read", { path: `project/spec/iterations/${id}/record.md` });
  assert.match(
...
test("the expedition seed is held to the same demand, and lands the same bytes", async () => {
  ...
  const rec = await call(server, "se_file_read", { path: `project/spec/expeditions/${id}/record.md` });
  assert.match(
    String((rec.body as { content?: string }).content ?? ""),
    /^ *\d+\tdepends_on: \[\]$/m,
