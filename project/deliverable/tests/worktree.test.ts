...
test("expeditions: worktree lifecycle — new, bind, work lands in the worktree, close merges", async () => {
  ...
  assert.equal(existsSync(join(root, "project", "spec", "expeditions", minted.created)), false, "the record dir left the tree");
  const rec = spawnSync("git", ["show", `exp/${minted.created}:project/spec/expeditions/${minted.created}/record.md`], { cwd: root, encoding: "utf8" }).stdout;
