...
const GUIDANCE = join(REPO_ROOT, "project", "guidance");
...
describe("no payload exceeds the tightest measured host limit", () => {
  ...
  test("EVERY served document fits, because the bound is under the tightest limit", () => {
    // This is the mechanism, and it is why two oversized guidance pages are
  ...
  test("the guidance corpus is measured, and what is over the raw line is named", () => {
    ...
    assert.ok(over.length <= 2, `more pages crossed the line than the 2 recorded on 2026-08-18: ${over.map((o) => o.name).join(", ")}`);
  });
});
