...
- THE LANE'S FIXER (`project/deliverable/engine/lintfix.ts`) runs Biome's SAFE fixes after
...
- THE SHELL BUNDLES: `project/deliverable/vscode/src/extension.ts` → esbuild →
...
thresholds live in `project/deliverable/machines/lint/voice-lint.md`; edit the file and the
...
The launcher's flags therefore live in `project/deliverable/engine/bin/se-mcp.ts` beside the
...
The renderer is `project/deliverable/engine/params.ts`. Every surface fetches the rendered bar
...
package state builds its archive by running `project/deliverable/engine/bin/package.ts` through
...
WHEN IT IS BUILT the verb wraps `project/deliverable/engine/bin/package.ts`, answers with the
